...
void HoParty::DrawImage(int Image, int x, int y, int w, int h)
{
...
void HoParty::friend_Insert(char* id, int Class)
{
	InterfaceParty_KTJ.chat_IDinsert(3, id, Class);
}
...
void HoParty::DrawPartyMessage(int x, int y)
{
	...
	if (MessageBoxOpenFlag == TRUE)
	{
		...
		DrawImage(LpButtonNo, BaseX + 120, BaseY + 110, 88, 35);//Botao Yes
		DrawImage(LpButtonYes, BaseX + 24, BaseY + 110, 88, 35);//Botao No
		...
		{
			DrawImage(LpButtonYes_, BaseX + 24, BaseY + 110, 88, 35);//Botao No
		...
		if (pRealCursorPos.x >= BaseX + 120 && pRealCursorPos.x <= BaseX + 120 + 88 &&
			pRealCursorPos.y >= BaseY + 110 && pRealCursorPos.y <= BaseY + 110 + 35)
		{
			DrawImage(LpButtonNo_, BaseX + 120, BaseY + 110, 88, 35);//Botao Yes
...
int HoParty::AddMember(char* memberName, DWORD chrCode, int life, int level, int JobCode)
{
	InterfaceParty_KTJ.AddMember(memberName, chrCode, life, level, JobCode);
	...
	PartyMember[PartyMemberCount].JobCode = JobCode;
...
int HoParty::AddMember(hoPartyMember* member, int cnt)
{
	...
	PartyMember[cnt].JobCode = member->JobCode;
...
void HoParty_KTJ::chatDATA_Insert(int menuN, char* str, int Class)
{
	int i, cnt;
	int l;
...
void HoParty_KTJ::chat_IDinsert(int sw, char* id, int Class)
{
	int i;
...
int HoParty_KTJ::AddMember(char* memberName, DWORD chrCode, int life, int level, int JobCode)
{
	if (memberName == NULL)
		return FALSE;
