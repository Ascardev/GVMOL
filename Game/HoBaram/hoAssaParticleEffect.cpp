...
int AssaParticle_HighLevel(smCHAR* pChar, int maxTime)
{
	if (GetAssaCodeEffect(pChar, HOLY_ACTION) == TRUE)
		return TRUE;
	...
	int i = GetAssaEffect();
	cASSAChar* partChar = new cASSAChar;
	partChar->Start(pChar, HIGH_LEVEL, maxTime * 70 * 60 * 60 * 60);
	cAssaEffect[i] = partChar;
...
int AssaParticle_ShootingEffect3(POINT3D* pPosi, POINT3D* pDesti)
{
	int i = GetAssaEffect();
	cASSAPARTSHOT* partShot = new cASSAPARTSHOT;
	partShot->Start(pPosi, pDesti, SKILL_T_CANNON);
	cAssaEffect[i] = partShot;

	g_NewParticleMgr.Start("Skill3_MartialArtist_TigerCannon_Start", *pPosi);

	g_NewParticleMgr.Start("Skill3_MartialArtist_TigerCannon_Hit", *pDesti);
...
void AssaParticle_FinalSpear2(smCHAR* pDest)
{
	POINT3D charPos;
	charPos.x = pDest->pX;
	charPos.y = pDest->pY;
...
void AssaParticle_GalaxyCoupShot(POINT3D* pCur, POINT3D* pDest, int count)
{
	for (int index = 0; index < count; index++)
	{
		POINT3D worldPos;
		GetMoveLocation(0, 10 * fONE, 0, 0, 0, ANGLE_360 / count * index);
		memcpy(&worldPos, pDest, sizeof(POINT3D));
		worldPos.x += GeoResult_X;
		worldPos.y += GeoResult_Y + 20000;
		worldPos.z += GeoResult_Z;
		...
		int i = GetAssaEffect();
		AssaSkill5AtalantaGalaxyCoup* GalaxyCoup = new AssaSkill5AtalantaGalaxyCoup;
		GalaxyCoup->Start(&worldPos, pDest, index * 10);
		cAssaEffect[i] = GalaxyCoup;
	}
}
...
void ParkKelvezuSkill2(smCHAR* pKelvezu)
{
	...
	curPos.y = pKelvezu->pY + 130000;
	curPos.z = pKelvezu->pZ + 50000;
	AssaParticle_TerrainFire(&curPos, &attackPos, 10, 0);
	...
	curPos.y = pKelvezu->pY + 130000;
	curPos.z = pKelvezu->pZ + 50000;
	AssaParticle_TerrainFire(&curPos, &attackPos, 20, 0);
	...
	curPos.y = pKelvezu->pY + 130000;
	curPos.z = pKelvezu->pZ + 50000;
	AssaParticle_TerrainFire(&curPos, &attackPos, 40, 0);
	...
	curPos.y = pKelvezu->pY + 130000;
	curPos.z = pKelvezu->pZ + 50000;
	AssaParticle_TerrainFire(&curPos, &attackPos, 50, 0);
	...
	curPos.y = pKelvezu->pY + 130000;
	curPos.z = pKelvezu->pZ + 50000;
	AssaParticle_TerrainFire(&curPos, &attackPos, 70, 0);
}
...
void AssaParticle_ShelltomWeaponStop(smCHAR* pChar)
{
	for (int i = 0; i < cASSAEFFECT_MAX; i++)
	{
		if (cAssaEffect[i])
...
void AssaParticle_TerrainFire(POINT3D* pCur, POINT3D* pDest, int delay, DWORD dwSkillCode)
{
	int i = GetAssaEffect();
	AssaSkyParticle* skyParticle = new AssaSkyParticle;
	skyParticle->Start(pCur, pDest, delay, SKILL_P_DEITY);
	cAssaEffect[i] = skyParticle;
}

void AssaParticle_TerrainSnow(POINT3D* pCur, POINT3D* pDest, int delay, DWORD dwSkillCode)
{
	int i = GetAssaEffect();
	AssaSkyParticle* skyParticle = new AssaSkyParticle;
	skyParticle->Start(pCur, pDest, delay, 0x50);
	cAssaEffect[i] = skyParticle;
}
...
void AssaParticle_DownHitEffect(smCHAR* pChar)
{
	POINT3D pos;

	if (pChar->GetAttackPoint(&pos.x, &pos.y, &pos.z))
	{
		SetDynLight(pos.x, pChar->pY, pos.z, 255, 100, 50, 0, 190, 3);
...
void AssaParticle_PiercingIce(smCHAR* pCurChar, smCHAR* pDestChar)
{
	POINT3D curPos;
	curPos.x = pCurChar->pX;
	curPos.y = pCurChar->pY + 5000;
	...
	{
		i = GetAssaEffect();
		AssaSkill5PiercingIce* PiercingIce = new AssaSkill5PiercingIce;
		...
		velocity.x += ((float)GeoResult_X * 2.f / (float)fONE);
		velocity.z += ((float)GeoResult_Z * 2.f / (float)fONE);
		PiercingIce->Start(&curPos, pDestChar, &velocity, 0);
		cAssaEffect[i] = PiercingIce;
	}
	else
	{
		i = GetAssaEffect();
...
void ParkAssaParticle_MonsterIgniculus(smCHAR* pChar)
{
	if (GetAssaCodeEffect(pChar, FIRE_MONSTER_IGNICULIS) == FALSE)
	{
		int i = GetAssaEffect();
		cAssaTraceCharMeshParticle* traceMeshParticle = new cAssaTraceCharMeshParticle;
		traceMeshParticle->Start(pChar, "Bip001 Spine", "IgniculusBody");
		cAssaEffect[i] = traceMeshParticle;
		cAssaEffect[i]->CODE = FIRE_MONSTER_IGNICULIS;
	}
}

void ParkAssaParticle_MonsterFlameMaiden(smCHAR* pChar)
{
	if (GetAssaCodeEffect(pChar, FIRE_MONSTER_FLAME_MAIDEN) == FALSE)
	{
		int i = GetAssaEffect();
		cAssaTraceCharMeshParticle* traceMeshParticle = new cAssaTraceCharMeshParticle;
		traceMeshParticle->Start(pChar, "Bip001 Spine", "FlamemaidenBody");
		cAssaEffect[i] = traceMeshParticle;
		cAssaEffect[i]->CODE = FIRE_MONSTER_FLAME_MAIDEN;
	}
}

void ParkAssaParticle_MonsterBossDraxos(smCHAR* pChar)
{
	if (GetAssaCodeEffect(pChar, BOSS_MONSTER_DRAXOS) == FALSE)
	{
		int i = GetAssaEffect();
		cAssaTraceCharMeshParticle* traceMeshParticle = new cAssaTraceCharMeshParticle;
		traceMeshParticle->Start(pChar, "Bip001 Spine", "DraxosBody");
		cAssaEffect[i] = traceMeshParticle;
		cAssaEffect[i]->CODE = BOSS_MONSTER_DRAXOS;
	}
}

void ParkAssaParticle_MonsterInsec(smCHAR* pChar)
{
	if (GetAssaCodeEffect(pChar, FIRE_MONSTER_INSEC) == FALSE)
	{
		int i = GetAssaEffect();
		cAssaTraceCharMeshParticle* traceMeshParticle = new cAssaTraceCharMeshParticle;
		traceMeshParticle->Start(pChar, "Bip001 Spine", "InsecBody");
		cAssaEffect[i] = traceMeshParticle;
		cAssaEffect[i]->CODE = FIRE_MONSTER_INSEC;
	}
}