...
int HoEffectPatDest::Main()
{
	...
	if (CurrentFrame < (AnimationEnd * 160 - 1) - 5 * FrameStep)
	{
		...
		if (SkillType == SKILL_PERFECT_AIM || SkillType == SKILL_WIND_ARROW || SkillType == MONSTER_WEB_SHOT1
			|| SkillType == SKILL_ARROW_OF_RAGE || SkillType == SKILL_C_TRAP || SkillType == SKILL_AVALANCHE || SkillType == SKILL_GROUND_PIKE)
		{
			LocalX += GeoResult_X;
			LocalY += GeoResult_Y;
			LocalZ += GeoResult_Z;
			...
			else if (SkillType == SKILL_C_TRAP)
			{
				if (length < 35000 && WorkState == PAT_PROCESS)
				{
					GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), DestX, DestY - 40000, DestZ);
				...
				if (length < 30000 && WorkState == PAT_PROCESS)
				{
					...
					x = int(WorldX + LocalX) + GeoResult_X;
					z = int(WorldZ + LocalZ) + GeoResult_Z;
					StartSkillDest(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), x, DestY - 40000, z, SKILL_C_TRAP_END, Level);
					return TRUE;
				}
			}
		...
		else
		{
			...
			if (WorkState == PAT_PROCESS && mapY1 > (WorldY + LocalY - 4000))
			{
				...
				else if (SkillType == SKILL_C_TRAP_END)
				{
					BlurBlendValue = 0;
					StartEffect(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), EFFECT_C_TRAP_HIT1);
				}
			}
		}
...
int HoEffectShield::Start(int x, int y, int z, int shieldType, int angle, int circleRadius, smCHAR* character)
{
	...
	if (shieldType == SKILL_SHIELD1)
	{
		...
		if (character->smCharInfo.JOB_CODE == 4)
			BackBillBoard.StartBillRect(x, y, z, "Skill5Pikeman_Amplified_ShieldParticle.ini", ANI_LOOP);
		else
