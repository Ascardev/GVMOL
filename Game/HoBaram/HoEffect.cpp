...
HoEffectMgr::~HoEffectMgr()
{
	...
	for (index = 0; index < MAX_EFFECTOBJECT_BUFFER; index++)
	{
	...
	for (index = 0; index < MAX_OBJECT_NUM; index++)
	{
...
void HoEffectMgr::Init()
{
	...
	PatObj[index] = smASE_Read(buffer);
	PatObj[index]->ZeroNormals();
	for (i = 0; i< int(PatObj[index]->smMaterialGroup->MaterialCount); i++)
		PatObj[index]->smMaterialGroup->smMaterial[i].BlendType = SMMAT_BLEND_LAMP;
	index++;
	...
	case SKILL_C_TRAP:
	{
		...
		for (int index = 0; index < 1; index++)
		{
			...
			if (patDest != NULL)
			{
				...
				patDest->StartDest(currentPos, destPos, PatObj[6], SKILL_C_TRAP, level);
				patDest->AnimationEnd = PatAnimationEnd[6];
	...
	case SKILL_C_TRAP_END:
	{
		...
		for (int index = 0; index < 10 + level / 2; index++)
		{
			...
			if (patDest != NULL)
			{
				...
				patDest->StartDest(currentPos, destPos, PatObj[6], SKILL_C_TRAP_END);
				patDest->AnimationEnd = PatAnimationEnd[6];

				AssaParticle_CircleTrap(&currentPos, &destPos, 3);
...
int HoEffectMgr::StartSkill(int x, int y, int z, int angleX, int angleY, int angleZ, int skillType, int level)
{
	...
	case SKILL_S_ROPE:
	{
		...
		for (int index = 0; index < 4; index++)
		{
			...
			if (primitiveBillboard != NULL)
			{
				primitiveBillboard->StartBillRect(x, y + 8000, z, "ThunderRoop2.ini", ANI_ONE);
	...
	case SKILL_PIKE_WIND:
	{
		...
		y = y + 500;
		for (index = 0; index < 15 + level; index++)
		{
			HoParticleDest* particleDest = new HoParticleDest;
...
int HoEffectMgr::StartSkill(int x, int y, int z, int angleX, int angleY, int angleZ, smCHAR* character, int skillType, int level)
{
	...
	case SKILL_SHIELD1:
	{
		...
		for (index = 0; index < CHR_IN_EFFECT_OBJECT_MAX; index++)
		{
...
int HoEffectMgr::Start(int x, int y, int z, int effectType, HoEffectObject* parent)
{
	...
	case EFFECT_S_IMPACT1:
	{
		...
		HoPrimitiveBillboard* primitiveBillBoard = new HoPrimitiveBillboard();
		primitiveBillBoard->StartBillRect(x, y, z, "SkillSpiritImpact2.ini", ANI_ONE);
		...
		for (int index = 0; index < 3; index++)
		{
			...
			if (billBoard != NULL)
			{
				...
				billBoard->StartDestPath(worldPos, localPos, destPos, size, "SkillSpiritImpact1.ini", 300);
...
int HoEffectMgr::Start(int x, int y, int z, int effectType, int level)
{
	...
	case EFFECT_C_TRAP_HIT1:
	{
		...
		for (int index = 0; index < 5; index++)
		{
			...
			currentPos.y = y - 2000;
			currentPos.z = z;
			g_NewParticleMgr.Start("Skill5ArcherCircleTrapHit", currentPos);
	...
	case EFFECT_S_IMPACT2:
	{
		...
		for (index = 0; index < 20 + level * 5; index++)
		{
			HoPrimitiveBillboard* billBoard = new HoPrimitiveBillboard();
			if (billBoard != NULL)
			{
				...
				billBoard->StartDestPath(worldPos, localPos, destPos, size, "SkillSpiritImpact1.ini", 500 + rand() % 400);
				if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(billBoard, index / 2))
				{
					delete billBoard;
					billBoard = NULL;
	...
	case SKILL_LAND_M:
	{
		...
		HoPrimitiveBillboard* extPrimitive = new HoPrimitiveBillboard();
		extPrimitive->StartBillRect(x, y, z, "Skill_Land_Mining_Effect1.ini", ANI_ONE);
		...
		extPrimitive = new HoPrimitiveBillboard();
		extPrimitive->StartBillRect(x, y, z, "Skill_Land_Mining_Effect2.ini", ANI_ONE);
	...
	case SKILL_SILRAPHIM:
	{
		...
		if (tracker != NULL)
		{
			...
			if (particleSystem != NULL)
			{
				...
				particleSystem->Start(currentPos.x, currentPos.y, currentPos.z, MaterialNum[2], SKILL_SILRAPHIM);

				tracker->Start(currentPos, destPos, SKILL_SILRAPHIM, particleSystem);
	...
	case SKILL_TENUS:
	{
		...
		if (tracker != NULL)
		{
			...
			if (particleSystem != NULL)
			{
				...
				particleSystem->Start(currentPos.x, currentPos.y, currentPos.z, MaterialNum[2], SKILL_TENUS);

				tracker->Start(currentPos, destPos, SKILL_TENUS, particleSystem);
...
int HoEffectMgr::StartMonster(POINT3D current, int effectType)
{
	...
	case SKILL_LAND_M:
	{
		...
		for (int index = 0; index < 20; index++)
		{
			...
			if (effectMove != NULL)
			{
				...
				effectMove->Start(cur, dest, SKILL_LAND_M);
	...
	case SKILL_SILRAPHIM:
	{
		...
		HoPrimitiveBillboard* primitiveBillBoard = new HoPrimitiveBillboard();
		primitiveBillBoard->StartPath(current, "SkillSilraphim1.ini", ANI_LOOP);
	...
	case SKILL_TENUS:
	{
		...
		HoPrimitiveBillboard* primitiveBillBoard = new HoPrimitiveBillboard();
		primitiveBillBoard->StartPath(current, "SkillVisTenus1.ini", ANI_LOOP);
...
int HoEffectTracker::Main()
{
	...
	if ((SkillCode == MONSTER_PIGON_SHOT1) ||
		...
		|| (SkillCode == EFFECT_FIRE_CRACKER) || (SkillCode == SKILL_SILRAPHIM)
		|| (SkillCode == SKILL_TENUS)
		)

	{
		if (EffectObj)
		{
			...
			if (length < 800 * 800 || liveCount > 3)
			{
				...
				if (SkillCode == SKILL_SILRAPHIM)
					StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, MONSTER_IMP_HIT2);

				if (SkillCode == SKILL_TENUS)
					StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, MONSTER_WEB_HIT1);
	...
	if ((SkillCode == SKILL_S_IMPACT))
	{
		if (length < 1000 * 1000)
		{
			StartFlag = FALSE;
			StartEffect(currentPos.x, currentPos.y, currentPos.z, EFFECT_S_IMPACT2, Level);
		...
		{
			sparkCount = 0;
			StartChildEffect(currentPos.x, currentPos.y, currentPos.z, EFFECT_S_IMPACT1, this);
		}
		...
		SkillCode == SKILL_TENUS)
	{
		...
		if (IMP_SHOT2_liveCount == 0)
		{
			...
			else if (SkillCode == SKILL_SILRAPHIM)
				StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, SKILL_SILRAPHIM);
			else if (SkillCode == SKILL_TENUS)
				StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, SKILL_TENUS);
		}
	}
...
int HoEffectTracker::Start(POINT3D currentPos, POINT3D destPos, int skillCode, HoEffectObject* effectObj, int level)
{
		...
		)
	{
		...
		if (SkillCode == MONSTER_PIGON_SHOT1 || SkillCode == MONSTER_FORZENMIST_SHOT1 || SkillCode == MONSTER_FORZENMIST_SHOT2 || SkillCode == SKILL_SILRAPHIM)
		{
			Vx += vx / length * 1500;
			Vy += vy / length * 1500;
			Vz += vz / length * 1500;
...
int HoNewEffectMove::Start(POINT3D curPos, POINT3D destPos, int skillCode)
{
	...
	if ((SkillCode == MONSTER_STYGIANLORD_PARTICLE1) || (SkillCode == MONSTER_COLDEYE_SKILL) || (SkillCode == SKILL_LAND_M))
	{
		Velocity.x = 0.0f;
		Velocity.y = 0.0f;
		Velocity.z = 0.0f;
...
int HoNewEffectMove::Main()
{
	...
	if (SkillCode == SKILL_LAND_M)
	{
		if (ParticleStartFlag == FALSE)
		{
			...
			data.y = int(WorldY + LocalY);
			data.z = int(WorldZ + LocalZ);
			PartEmitterID = g_NewParticleMgr.Start("Skill5_Mecanician_LandMining", data);
			ParticleStartFlag = TRUE;
		}
...
int InitEffect()
{
	...
	AnimDataMgr.ImageLoad("Effect\\ImageData\\Skill5Pikeman_Amplified_ShieldParticle.ini");
	...
	AnimDataMgr.ImageLoad("Effect\\ImageData\\Skill_Land_Mining_Effect1.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\Skill_Land_Mining_Effect2.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\SkillSpiritImpact1.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\SkillSpiritImpact2.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\SkillMotalBlowHit.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\ThunderRoop.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\ThunderRoop2.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\CircleTrap.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\SkillSilraphim1.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\SkillVisTenus1.ini");
	...
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\Skill5Pikeman_Amplified_ShieldParticle.ini");
	...
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\Skill_Land_Mining_Effect1.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\Skill_Land_Mining_Effect2.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\SkillSpiritImpact1.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\SkillSpiritImpact2.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\SkillMotalBlowHit.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\ThunderRoop.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\ThunderRoop2.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\CircleTrap.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\SkillSilraphim1.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\SkillVisTenus1.ini");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Force.part");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3Hit7.part");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\TOPPVP.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\HighLevelUserMerit_Star.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\EventPK.part");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ShadowBomb.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\RisingSlash1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\RisingSlash2.part");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Storm.part");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\CreedStart.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\CreedKeep.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\PressOfDeity1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\PressOfDeity2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GhostyNail1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GhostyNail2.part");

	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill1_MartialArtist_LowKick.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill1_MartialArtist_D_Backspin_Blow.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill1_MartialArtist_HyperStraight.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill2_MartialArtist_RageUp_Start.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill2_MartialArtist_RageUp_End.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill2_MartialArtist_RageUp_Keep.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill2_MartialArtist_Patriot_Start.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill2_MartialArtist_Patriot_Hit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill2_MartialArtist_RushElbow_Hit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_I_Bulk_Up_Start.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_I_Bulk_Up_Keep.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_Jumping_HeelKick1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_Jumping_HeelKick2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_WarCry_Start.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_WarCry_Hit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_WarCry_Keep.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_TigerCannon_Start.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill3_MartialArtist_TigerCannon_Hit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill4_MartialArtist_Combination_Jab.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill4_MartialArtist_Combination_Uppercut.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill4_MartialArtist_Combination_Straight.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill4_MartialArtist_TyphoonHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill4_MartialArtist_TyphoonHit2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill4_MartialArtist_BodyCheckHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill4_MartialArtist_BodyCheckHit2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill4_MartialArtist_Steelers.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_MartialArtist_HuntingHawkHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_MartialArtist_HuntingHawkHit2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_MartialArtist_HuntingHawkHit3.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_MartialArtist_LineBreakingHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_MartialArtist_LineBreakingHit2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_MartialArtist_HardTrainingStart.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_MartialArtist_HardTrainingKeep.part");

	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Mecanician_HyperSonic1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Mecanician_HyperSonic2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Mecanician_LandMining.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Mecanician_PowerEnhance1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Mecanician_PowerEnhance2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Meca_PowerEnhance_Start.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Meca_RollingSmash_Hit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Meca_RollingSmash_Down.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Fighter_DownHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Fighter_BloodyBerserker.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Fighter_PowerDash.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\\Skill5_Atalanta_Talaria_StartEffect.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Atalanta_Talaria_EndEffect.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Atalanta_Talaria_KeepEffect.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Atalanta_SummonArcuda_Effect.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Acher_Evade_ShotEffect.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Acher_EvadeShot2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Acher_EvadeShot3.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Acher_EvadeShot4.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5ArcherThubderRoopShot2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\script\\Skill5ArcherCircleTrap1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5ArcherCircleTrapHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Knight_CrescentMoon1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Knight_CrescentMoon2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Knight_SaintBlade1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Knight_SaintBlade2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5KnightHolyBenedicHand1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5KnightHolyBenedicHand2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Knight_HolyBenedic.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Knight_HolyBenedic2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5PriestessPiercingIce1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5PriestessPiercingIce2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5PriestessPiercingIceHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5PriestessRamielHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5PriestessRamielHand.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5PriestessRamielStart.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5PriestessRamielEnd.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\script\\Skill5PriestessRamiel.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\S5_Pr_BlessingKrishna_Effect.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\S5_Pr_BlessingKrishna.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5MagicianPrimaIgnisShot.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_Atalanta_Galaxy_CoupEffect.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5AtalantaGalaxyCoup.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5AtalantaSnippyFearShot.part");

	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_PikeMan_FinalSpear1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_PikeMan_FinalSpear2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_PikeMan_FinalSpear3.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5_PikeMan_FinalSpear4.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Skill5PikemanAmplified.part");

	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\IgniculusBody.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\IgniculusHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\IgniculusSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaArmorHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaArmorStart.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaArmorSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaArcherHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaArcherStart.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaArcherSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\InsecBody.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\InsecHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\InsecSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaGolemHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaGolemSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaGiantHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaGiantStart.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LavaGiantSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\FlamoHit1.part.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\FlamoHit2.part.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\FlamoSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\FlamemaidenBody.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\FlamemaidenHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\FlamemaidenStart.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\FlamemaidenSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DraxosBody.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DraxosHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DraxosSkill1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DraxosSkill2.part");
