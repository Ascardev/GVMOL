...
char* szClowImage2 = "weapons\\crown2.tga";
char* szClowImage3 = "weapons\\crown3.tga";
...
int InitPat3D()
{
	...
	if (PatCrown2) {
		...
		mat = PatCrown2->smMaterialGroup->FindMaterialFromTextureName(szClowImage2);
		if (mat) {
			mat->BlendType = SMMAT_BLEND_ADDCOLOR;
			mat->SelfIllum = 1;
		}
	...
	if (PatCrown3) {
		...
		mat = PatCrown3->smMaterialGroup->FindMaterialFromTextureName(szClowImage3);
		if (mat) {
			mat->BlendType = SMMAT_BLEND_ADDCOLOR;
			mat->SelfIllum = 1;
		}
...
int PlayPat3D()
{
			...
			if (lpCharMsTrace->smCharInfo.Life[0] <= 0 ||
				...
				if (lpAttackSkill) {
					if (lpAttackSkill->CODE == SKILL_HEALING || lpAttackSkill->CODE == SKILL_ENCHANT_WEAPON ||
						lpAttackSkill->CODE == SKILL_VIRTUAL_LIFE || lpAttackSkill->CODE == SKILL_TRIUMPH_OF_VALHALLA || lpAttackSkill->CODE == SKILL_H_BENEDIC)
					{
								...
								if (lpCurPlayer->ShootingMode && lpCharMsTrace->smCharInfo.State != smCHAR_STATE_NPC) {
									...
									if (lpCharMsTrace->smCharInfo.State == smCHAR_STATE_NPC) {
										...
										if (dwClassDelayTime < dwPlayTime)
										{
											...
											{
												Quest::GetInstance()->sinCheckQuest(lpCharMsTrace->smCharInfo.wPlayClass[0]);
					...
					if ((lpMsTraceItem->ItemCode & sinITEM_MASK2) == sinCH1)
					{
						...
						case (sinCH1 | sin02):
							...
							if (lpCurPlayer->smCharInfo.JOB_CODE != JOBCODE_ATALANTA &&
								...
								lpCurPlayer->smCharInfo.JOB_CODE != JOBCODE_MARTIAL &&
								lpCurPlayer->smCharInfo.JOB_CODE != JOBCODE_PRIESTESS)
							{
								cMessageBox.ShowMessage(MESSAGE_NOT_PICKUP_FEMALE);
								cnt = 1;
	...
	if ((PlayCounter & 0x3) == 0) {
		...
		if ((PlayCounter & 0xFF) == 0)
		{
			...
			if (!lpCurPlayer->smCharInfo.dwEventTime_T && BigHead)
			{
				BigHead = 0;
			...
			if (!DisconnectFlag && RecordFailCount >= 6) {
				...
				if (smConfig.DebugMode) AddChatBuff("DC12");
#ifdef _WINMODE_DEBUG
...
int DrawPat3D(int x, int y, int z, int ax, int ay, int az)
{
	...
	if (cShop.OpenFlag || cCharShop.OpenFlag)
	{
		...
		if (CursorClass == CursorID::Attack ||
			CursorClass == CursorID::Talk ||
			CursorClass == CursorID::GetItem1 ||
			CursorClass == CursorID::GetItem2)
		{
	...
	else
	{
		...
		if (lpCharSelPlayer)
		{
			...
			case smCHAR_STATE_ENEMY:
				SetCursorGame(CursorID::Attack);
				break;
			case smCHAR_STATE_USER:
				break;
			case smCHAR_STATE_NPC:
				SetCursorGame(CursorID::Talk);
				break;
			}
		}
		...
		{
			if (MouseButton[0])
				SetCursorGame(CursorID::GetItem2);
			else
				SetCursorGame(CursorID::GetItem1);
		}
	}
	...
	if (chrEachPlayer.Flag && chrEachMaster && chrEachMaster->Flag)
	{
		...
		if (SizeMode) {
			if (chrEachPlayer.szChatMessage[0]) {
				...
				if (GameMode == 2)
				{
					if (lpCurPlayer) if (StageField[lpCurPlayer->OnStageField]->FieldCode != 43)
			...
			dist = GetTextFullSize(chrEachPlayer.smCharInfo.szName) >> 1;

			SetFontTextColor(RGB(0, 0, 0));
			if (GameMode == 2) { if (lpCurPlayer) if (StageField[lpCurPlayer->OnStageField]->FieldCode != 43) dsTextLineOut(MidX - dist + 1, MidY - (h >> 1) + h - 33, chrEachPlayer.smCharInfo.szName, len); }
			else dsTextLineOut(MidX - dist + 1, MidY - (h >> 1) + h - 33, chrEachPlayer.smCharInfo.szName, len);
			SetFontTextColor(RGB(255, 255, 180));
			if (GameMode == 2) { if (lpCurPlayer) if (StageField[lpCurPlayer->OnStageField]->FieldCode != 43) dsTextLineOut(MidX - dist, MidY - (h >> 1) + h - 32, chrEachPlayer.smCharInfo.szName, len); }
			else dsTextLineOut(MidX - dist, MidY - (h >> 1) + h - 32, chrEachPlayer.smCharInfo.szName, len);
		}
		else {
			if (!EachTradeButton) {
				...
				else if (lpCurPlayer->OnStageField >= 0 && StageField[lpCurPlayer->OnStageField]->State == FIELD_STATE_CASTLE)
				{
					if (chrEachMaster->smCharInfo.State == smCHAR_STATE_USER || chrEachMaster->PkMode_CharState == smCHAR_STATE_USER)
					{
						chaSiege.ShowPickUserKillPoint(MidX - 32, MidY + (h >> 1) + 10, chrEachMaster->BattlePoint);
						chaSiege.ShowPickUserDeadPoint(MidX - 32, MidY + (h >> 1) + 30, chrEachMaster->DeathPoint);
					}
				}
			}
			...
			if (chrEachPlayer.smCharInfo.ClassClan && chrEachPlayer.ClanInfoNum >= 0) {
				...
				if (GameMode == 2)
				{
					if (StageField[lpCurPlayer->OnStageField]->FieldCode != 43)
					{
						DrawClanMark_EachBox(ClanInfo[chrEachPlayer.ClanInfoNum].hClanMark32, ClanX - 25, ClanY);
					}
				}
				else
				{
			...
			if (smConfig.DebugMode && cInterFace.sInterFlags.MapOnFlag) {
				...
				dsTextLineOut(MidX - 50, ly, szBuff, lstrlen(szBuff));
				ly += 16;
				wsprintf(szBuff, "BattlePoint  : %d ", chrEachPlayer.BattlePoint);
				dsTextLineOut(MidX - 50, ly, szBuff, lstrlen(szBuff));
				ly += 16;
				wsprintf(szBuff, "DeathPoint  : %d ", chrEachPlayer.DeathPoint);
				dsTextLineOut(MidX - 50, ly, szBuff, lstrlen(szBuff));
				ly += 16;
