...
bool Firewall::AddRule(Firewall::Rule Rule, std::string IP, unsigned short Port, int Attack)
{
	...
	case Firewall::Rule::BlockAccess:
	{
		...
		if (Attack == 1)
		{
			RuleName = SysAllocString(L"WPT Ban Attack");
		}
		else
		{
			RuleName = SysAllocString(L"PT Server Firewall");
		}
