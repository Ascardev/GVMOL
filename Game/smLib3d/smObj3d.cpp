...
int smOBJ3D::LoadFile(smFileLoader& FileLoader, smPAT3D* PatPhysique /*= 0*/)
{
	try
	{
		FileLoader.Read(&Head, sizeof(*this), 1);

		smTEXLINK* lpOldTexLink = TexLink;

		Vertex = new smVERTEX[nVertex]();
		FileLoader.Read(Vertex, sizeof(smVERTEX), nVertex);

		Face = new smFACE[nFace]();
		FileLoader.Read(Face, sizeof(smFACE), nFace);

		TexLink = new smTEXLINK[nTexLink]();
		FileLoader.Read(TexLink, sizeof(smTEXLINK), nTexLink);

		TmRot = new smTM_ROT[TmRotCnt]();
		FileLoader.Read(TmRot, sizeof(smTM_ROT), TmRotCnt);

		TmPos = new smTM_POS[TmPosCnt]();
		FileLoader.Read(TmPos, sizeof(smTM_POS), TmPosCnt);

		TmScale = new smTM_SCALE[TmScaleCnt]();
		FileLoader.Read(TmScale, sizeof(smTM_SCALE), TmScaleCnt);

		TmPrevRot = new smFMATRIX[TmRotCnt]();
		FileLoader.Read(TmPrevRot, sizeof(smFMATRIX), TmRotCnt);

		for (int i = 0; i < nTexLink; i++)
			if (TexLink[i].NextTex)
				TexLink[i].NextTex = TexLink + (TexLink[i].NextTex - lpOldTexLink);

		for (int i = 0; i < nFace; i++)
			if (Face[i].lpTexLink)
				Face[i].lpTexLink = TexLink + (Face[i].lpTexLink - lpOldTexLink);

		if (PatPhysique)
		{
			Physique = new smOBJ3D * [nVertex]();

			char* Buffer = new char[nVertex * 32]();
			FileLoader.Read(Buffer, 32, nVertex);

			for (int i = 0; i < nVertex; i++)
			{
				Physique[i] = PatPhysique->GetObjectFromName(Buffer + i * 32);
			}
...
int smPAT3D::LoadFromBuffer(char* Buffer, size_t Size, char* szNodeName /*= 0*/)
{
	Init();

	TmParent = smASE_GetPhysique();

	try
	{
		smFileLoader FileLoader(Buffer, Size);

		smDFILE_HEADER FileHeader = { 0 };
		FileLoader.Read(&FileHeader, sizeof(FileHeader), 1);

		if (_strcmpi(FileHeader.szHeader, szSMDFileHeader) != 0)
		{
			char Message[256] = { 0 };
			sprintf_s(Message, "[Client] : File( %s ) does not have a compatible header format.", "Custom from Assets");
			throw std::runtime_error(Message);
		}

		if (FileHeader.ObjCounter == 0 && FileHeader.MatCounter == 0)
		{
			char Message[256] = { 0 };
			sprintf_s(Message, "[Client] : File( %s ) does not have any information to load.", "Custom from Assets");
			throw std::runtime_error(Message);
		}

		smDFILE_OBJINFO* FileObjInfo = new smDFILE_OBJINFO[FileHeader.ObjCounter]();
		FileLoader.Read(FileObjInfo, sizeof(smDFILE_OBJINFO), FileHeader.ObjCounter);

		TmFrameCnt = FileHeader.TmFrameCounter;
		CopyMemory(&TmFrame, FileHeader.TmFrame, sizeof(smFRAME_POS) * OBJ_FRAME_SEARCH_MAX);

		if (FileHeader.MatCounter)
		{
			smMaterialGroup = new smMATERIAL_GROUP();
			smMaterialGroup->LoadFile(FileLoader);
		}
		...
		if (szNodeName)
		{
			...
			{
				if (_strcmpi(szNodeName, FileObjInfo[i].szNodeName) == 0)
				{
					smOBJ3D* Object = new smOBJ3D();
					FileLoader.Seek(FileObjInfo[i].ObjFilePoint);
					Object->LoadFile(FileLoader, TmParent);
					AddObject(Object);
					break;
				}
			}
