	...
	while (!feof(fp))
	{
		...
		if (lstrcmp(decode, "*MATERIAL_NAME") == 0)
		{
			...
			if (strLevel == 1)
			{
				...
				{
					MESSAGE("MATERIAL Counter Error (ReadAse)");
			...
			if (SubPoint)
			{
				if (s_LightMapRefBuff[cnt].bLightMap)
				{
					...
					for (int scrNum = 0; scrNum < aseScriptCnt; scrNum++)
					{
						...
						{
							MESSAGE("ASESCRIPT MATNUM Error (ReadAse)");
...
smPAT3D* smReadModel_Assets(char* File, char* szNodeName = nullptr)
{
	char FileName[64] = { 0 };
	char FileDirectory[256] = { 0 };
	char FilePath[256] = { 0 };

	if (_splitpath_s(File, nullptr, 0, FileDirectory, sizeof(FileDirectory), FileName, sizeof(FileName), nullptr, 0) == 0)
	{
		sprintf_s(FilePath, "Assets\\Models\\%s.dat", FileName);
	}
	...
	if (Game::AssetsReader::GetInstance().Get(FilePath, Binary))
	{
		...
		if (Pat->LoadFromBuffer(const_cast<char*>(reinterpret_cast<const char*>(Binary.Memory)), Binary.Size, szNodeName))
		{
			return Pat;
		}
		else
...
smPAT3D* smASE_Read(char* file, char* szModelName)
{
	...
	if (_strnicmp(file, "image\\sinimage\\items\\dropitem\\", 30) == 0 || _strnicmp(file, "char\\tmabcd\\", 12) == 0 || _strnicmp(file, "char\\monster\\", 13) == 0 || _strnicmp(file, "char\\npc\\", 9) == 0)
	{
		smPAT3D* pModel = smReadModel_Assets(file, szName);
