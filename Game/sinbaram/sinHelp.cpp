...
void cSINHELP::LButtonDown(int x, int y)
{
	...
	for (int i = 0; i < SIN_MAX_HELP_NUMBER; i++)
	{
		if (sSinHelp[i].KindFlag)
		{
			...
			if (sSinHelp[i].KindFlag == NPC_ERAGONCAVE)
			{
				if (lpCurPlayer->smCharInfo.Level >= 110)
				{
					cMessageBox.ShowMessage3(MESSAGE_ERAGON_CAVE, "Desafio");
				}
				else
				{
					cMessageBox.ShowMessageEvent("VocÍ n„o tem nÌvel suficiente!");
				}
			}

			if (sSinHelp[i].KindFlag == NPC_BATTLEARENA)
			{
				if (BattleArena::GetInstance()->GetFlag() == 1)
				{
					cMessageBox.ShowMessage3(MESSAGE_BATTLE_ARENA, "Desafio");
				}
			}
