...
int ShopGoldEdit[2][4] = {
	{231,210 + 5 + sinInterHeight2,231 + 120,213 + 5 + 24 + sinInterHeight2},
	{224,240 + 5 + sinInterHeight2,224 + 120,243 + 5 + 24 + sinInterHeight2},
};
...
void cSHOP::Load()
{
	...
	lpRemodelTitle = LoadDibSurfaceOffscreen("Image\\SinImage\\Shopall\\CraftItem\\RemodelTitle.bmp");
	lpRemodelButtonInfo = LoadDibSurfaceOffscreen("Image\\SinImage\\Shopall\\CraftItem\\RemodelButtonInfo.bmp");
...
void cSHOP::Release()
{
	...
	if (lpRemodelTitle)
	{
		lpRemodelTitle->Release();
		lpRemodelTitle = 0;
	}

	if (lpRemodelButtonInfo)
	{
		lpRemodelButtonInfo->Release();
		lpRemodelButtonInfo = 0;
	}
}
...
void cSHOP::Draw()
{
	...
	if (cMyShop.OpenFlag) {
		...
		dsTextLineOut(CheckEditSize(ShopGoldEdit[0][0], ShopGoldEdit[0][2], strBuff) - (256 + 128 - sinMoveKindInter[SIN_MYSHOP]) - (len3 / 2), ShopGoldEdit[0][1],
			strBuff, lstrlen(strBuff));
	}
			...
			if (cMyShop.MyShopItem[i].Flag) {
				if (cMyShop.MyShopItem[i].x < pCursorPos.x && cMyShop.MyShopItem[i].x + cMyShop.MyShopItem[i].w > pCursorPos.x &&
					...
					if (Index2) {
						...
						MyShopItemSellMoney2 = cMyShop.MyShopItem[i].sItemInfo.Coin ? cMyShop.MyShopItem[i].sItemInfo.Coin : cMyShop.MyShopItem[i].sItemInfo.Money;
						KindCurrency = cMyShop.MyShopItem[i].sItemInfo.Coin ? 1 : 2;
			...
			if (cCharShop.CharShopItem[i].Flag) {
				if (cCharShop.CharShopItem[i].x < pCursorPos.x && cCharShop.CharShopItem[i].x + cCharShop.CharShopItem[i].w > pCursorPos.x &&
					...
					if (Index2) {
						...
						KindCurrency = sRecvMyShop.sMyShopItem[Index2 - 1].KindCurrency;
...
void cSHOP::LButtonDown(int x, int y)
{
	...
	if (ShowShopItemIndex)
	{
		if (CursorClass == CursorID::Default || CursorClass == CursorID::Buy)
		{
	...
	case 2:
		if (sinShopKind == 3)break;
		SetCursorGame(CursorID::Repair);
		break;

	case 3:
		SetCursorGame(CursorID::Sell);
		break;

	case 4:
		SetCursorGame(CursorID::Buy);
		break;
...
int cSHOP::SellItemToShop(sITEM * pItem, int ItemCount)
{
	...
	else
	{
		...
		case 2:
			...
			break;
		case 3:
			if ((sinChar->Money + pItem->SellPrice * ItemCount) >= 1000000000)
			{
				cMessageBox.ShowMessage(MESSAGE_OVER_MONEY);
				cInvenTory.ReFormInvenItem();
				return FALSE;
	...
	if (pItem->Class == ITEM_CLASS_WEAPON_TWO && CursorClass == CursorID::Sell)
...
void cSHOP::DrawShopText()
{
	...
	if (OpenFlag) {
		...
		dsTextLineOut(CheckEditSize(ShopGoldEdit[1][0], ShopGoldEdit[1][2], strBuff) - (256 + 128 - sinMoveKindInter[SIN_SHOP]), ShopGoldEdit[1][1],
			strBuff, lstrlen(strBuff));
		...
		if (ShopItemPrice) {
			...
			dsTextLineOut(CheckEditSize(ShopGoldEdit[0][0], ShopGoldEdit[0][2], strBuff) - (256 + 128 - sinMoveKindInter[SIN_SHOP]), ShopGoldEdit[0][1],
				strBuff, lstrlen(strBuff));
		}
	}
	int BackUpPosX, BackUpPosY = 0;
			...
			if (index) {
				for (j = 0; j < 30; j++) {
					if (!sMyShop.sMyShopItem[j].sItem.CODE) {
						sMyShop.sMyShopItem[j].Price = cMyShop.MyShopItem[i].sItemInfo.Coin ? cMyShop.MyShopItem[i].sItemInfo.Coin : cMyShop.MyShopItem[i].sItemInfo.Money;
						sMyShop.sMyShopItem[j].KindCurrency = cMyShop.MyShopItem[i].sItemInfo.Coin ? 1 : 2;
						...
						if (lpShopItem->CkSum == cInvenTory.InvenItem[index - 1].sItemInfo.ItemHeader.dwChkSum &&
							...
							if (MASK_CODE == sinPM1 || MASK_CODE == sinPL1 || MASK_CODE == sinPS1) {
								if (lpShopItem->Price != sMyShop.sMyShopItem[index5 - 1].Price * lpShopItem->sTime) {
									...
									sMyShop_Server.KindCurrency = sMyShop.sMyShopItem[index5 - 1].KindCurrency;
...
int cCHARSHOP::BuyItem(int Index)
{
	...
	if (Index2) {
		...
		sMyShop_Server.KindCurrency = sRecvMyShop.sMyShopItem[Index2 - 1].KindCurrency;
		lstrcpy(sMyShop_Server.szName, sRecvMyShop.sMyShopItem[Index2 - 1].sItem.ItemName);
		...
		if (sRecvMyShop.sMyShopItem[Index2 - 1].KindCurrency == 1) {
			if ((CoinShop::GetInstance()->GetCoin() - (int)sMyShop_Server.Price) >= 0) {
				if (cInvenTory.CheckInvenEmpty(&TempItem)) {
				...
				cMessageBox.ShowMessageEvent("Você não tem créditos suficientes!");
			}
		}
		else {
...
int cMYSHOP::GetLimitMoney(int Money)
{
	...
	if (sinChar->Level <= 10) {
		...
		case 2:
			...
			break;
		case 3:
			return ((1000000000) - (Money + sinChar->Money));
			break;
		case 4:
			return ((1000000000) - (Money + sinChar->Money));
			break;
