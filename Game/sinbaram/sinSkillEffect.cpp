...
#define SIN_PARTICLE_SILRAPHIM		10017
#define SIN_PARTICLE_TENUS   		10018
...
void sinInitSkillEffect()
{
	...
	for (i = 0; i < 9; i++) {
		wsprintf(szBuff, "%s\\Silraphim\\Silraphim%d.tga", Path, i + 1);
		MatSilraphim[i] = CreateTextureMaterial(szBuff, 0, 0, 0, 0, SMMAT_BLEND_LAMP);
	}

	for (i = 0; i < 5; i++) {
		wsprintf(szBuff, "%s\\ThirdAnima\\%dos.tga", Path, i + 1);
		MatThirdAnima[i] = CreateTextureMaterial(szBuff, 0, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
}
...
void sinSkillEffectMove(cSINEFFECT2* pEffect)
{
	...
	case SKILL_TENUS:
		...
		//sinEffect_VisTenus_Particle(&Posi);
	...
	case SKILL_SILRAPHIM:
		...
		sinEffect_Silraphim_Particle(&Posi);
	...
	case SIN_PARTICLE_SILRAPHIM:
		pEffect->sinFace.x += pEffect->MoveSpeed.x;
		pEffect->sinFace.z += pEffect->MoveSpeed.z;

		if (pEffect->ActionTime[0] <= pEffect->Time)
		{
			pEffect->sinFace.MatNum = MatSilraphim[3];
			pEffect->sinFace.y += pEffect->MoveSpeed.y * 2;
		}
...
int sinEffect_Silraphim(smCHAR* pChar)
{
	if (!pChar)return FALSE;
	int Size;
	SetDynLight(pChar->pX, pChar->pY, pChar->pZ, 255, 255, 255, 255, 200, 1);
	...
	cSinEffect2[Index2].sinFace.height = Size;
	cSinEffect2[Index2].sinFace.width = Size;
	cSinEffect2[Index2].CODE = SKILL_SILRAPHIM;
	cSinEffect2[Index2].Max_Time = 70;
	cSinEffect2[Index2].AniCount = 1;
	cSinEffect2[Index2].AniMax = 4;
	cSinEffect2[Index2].AniTime = 10;
...
int sinEffect_VisTenus(smCHAR* pChar)
{
	if (!pChar)return FALSE;
	int Size;
	SetDynLight(pChar->pX, pChar->pY, pChar->pZ, 255, 255, 255, 255, 200, 1);
	Index2 = sinSearchEmptyIndex();
	sinEffectDefaultSet(Index2, SIN_EFFECT_FACE, pChar, 0, 0);
	cSinEffect2[Index2].sinFace.MatNum = MatAgony[6];
	Size = 2000;
	cSinEffect2[Index2].sinFace.height = Size;
	cSinEffect2[Index2].sinFace.width = Size;
	cSinEffect2[Index2].CODE = SKILL_TENUS;
	cSinEffect2[Index2].Max_Time = 70;
	cSinEffect2[Index2].AniCount = 1;
	cSinEffect2[Index2].AniMax = 4;
	cSinEffect2[Index2].AniTime = 10;
...
int sinEffect_ThirdAnima(smCHAR* pChar, int Time)
{
	if (!pChar)return FALSE;
	int TempX = 0, TempZ = 0, TempSize;
	SetDynLight(pChar->pX, pChar->pY, pChar->pZ, 255, 255, 255, 255, 200, 1);
	for (int i = 0; i < 5; i++)
	{
		Index2 = sinSearchEmptyIndex();
		sinEffectDefaultSet(Index2, SIN_EFFECT_MESH, pChar, 0, 7000);
		cSinEffect2[Index2].ObjMat = MatThirdAnima[i];
		cSinEffect2[Index2].pChar = pChar;
		cSinEffect2[Index2].CODE = SKILL_ANIMA;
		cSinEffect2[Index2].Max_Time = Time * 70;
		cSinEffect2[Index2].Index = i + 1;
		TempSize = 500;
		cSinEffect2[Index2].RotateDistance.z = 256 * 8 * 2;
...
int sinEffect_Silraphim_Particle(POINT3D* pPosi)
{
	int Size;
	for (int i = 0; i < 9; i++)
	{
		...
		cSinEffect2[Index2].sinFace.height = Size;
		cSinEffect2[Index2].sinFace.width = Size;
		cSinEffect2[Index2].sinFace.MatNum = MatSilraphim[1];
		cSinEffect2[Index2].Max_Time = 20;
		cSinEffect2[Index2].Angle.x = ((rand() % (4096 / 2)) - ANGLE_90) & ANGCLIP;
		cSinEffect2[Index2].Angle.z = 0;
		cSinEffect2[Index2].Angle.y = rand() % 4096;
		cSinEffect2[Index2].Time -= i;
		cSinEffect2[Index2].CODE = SIN_PARTICLE_SILRAPHIM;
...
int sinEffect_VisTenus_Particle(POINT3D* pPosi)
{
	/*
	Index2 = sinSearchEmptyIndex();
	sinEffectDefaultSet(Index2, SIN_EFFECT_FACE, 0, pPosi, 0);
	cSinEffect2[Index2].Max_Time = 100;
	cSinEffect2[Index2].CODE = SIN_PARTICLE_TENUS;
	cSinEffect2[Index2].sinFace.x += rand() % 100;
	cSinEffect2[Index2].sinFace.y += rand() % 100;
	cSinEffect2[Index2].sinFace.z += rand() % 100;
	cSinEffect2[Index2].MoveSpeed.x += rand() % 3;
	cSinEffect2[Index2].MoveSpeed.y += rand() % 50;
	cSinEffect2[Index2].MoveSpeed.z += rand() % 3;
	*/
...
void sinEffect_StartMagic2(POINT3D* pPosi, int Type)
{
	switch (Type)
	{
	case 1:
		Assa = SetAssaEffect(0, "MAAM3.ASE", 0, pPosi, 1500);
		cAssaEffect[Assa]->AniMaxCount = 20;
		cAssaEffect[Assa]->AniDelayTime = 4;
		Assa = SetAssaEffect(160, "warp_magicsquare01a.tga", 0, pPosi, 1500);
		cAssaEffect[Assa]->FACE_TYPE = ASSAFACE_WORLD;
		cAssaEffect[Assa]->Size.w = 26500;
		Assa = SetAssaEffect(160, "star05C_03.bmp", 0, pPosi, 1500);
		cAssaEffect[Assa]->FACE_TYPE = ASSAFACE_WORLD;
		cAssaEffect[Assa]->Size.w = 31000;
		cAssaEffect[Assa]->MaxAlphaAmount = 120;
		break;
	case 2:
		Assa = SetAssaEffect(0, "MAAM4.ASE", 0, pPosi, 1500);
		cAssaEffect[Assa]->AniMaxCount = 20;
		cAssaEffect[Assa]->AniDelayTime = 4;
		Assa = SetAssaEffect(160, "vesper06_1.bmp", 0, pPosi, 1500);
		cAssaEffect[Assa]->FACE_TYPE = ASSAFACE_WORLD;
		cAssaEffect[Assa]->Size.w = 26500;
		Assa = SetAssaEffect(160, "star05C_03.bmp", 0, pPosi, 1500);
		cAssaEffect[Assa]->FACE_TYPE = ASSAFACE_WORLD;
		...
		break;
	case 3:
		Assa = SetAssaEffect(0, "MAAM5.ASE", 0, pPosi, 1500);
		cAssaEffect[Assa]->AniMaxCount = 20;
		cAssaEffect[Assa]->AniDelayTime = 4;
		Assa = SetAssaEffect(160, "N_summon_01_1.bmp", 0, pPosi, 1500);
		cAssaEffect[Assa]->FACE_TYPE = ASSAFACE_WORLD;
		cAssaEffect[Assa]->Size.w = 26500;
		Assa = SetAssaEffect(160, "star05C_03.bmp", 0, pPosi, 1500);
		cAssaEffect[Assa]->FACE_TYPE = ASSAFACE_WORLD;
