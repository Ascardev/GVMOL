...
static int GetWareHouseFile(char* szName, int Page, char* szFileName)
{
	char* szID = szName;
	...
	if (!Page)
		...
		wsprintf(szFileName, "DataServer\\%s\\%d\\%s.war%d", szRecordWareHouseDir, GetUserCode(szName), szName, Page);
...
static int GetWareHouseFile_Backup(char* szName, int Page, char* szFileName, int Day)
{
	char* szID = szName;
	char szBuff[256];
	...
	if (!Page)
		...
		wsprintf(szFileName, "%s\\%s\\%d\\%s.war%d", szBuff, szRecordWareHouseDir, GetUserCode(szName), szName, Page);
...
int	RecordSkill(RECORD_SKILL* lpRecordSkill)
{
	...
	for (cnt = 0; cnt < SIN_MAX_USE_SKILL; cnt++) {
		lpRecordSkill->bSkillPoint[cnt - 1] = sinSkill.UseSkill[cnt].Point;
		if (sinSkill.UseSkill[cnt].Point > 255) lpRecordSkill->bSkillPoint[cnt - 1] = 255;

		if (sinSkill.UseSkill[cnt].UseSkillCount < 10000)
			lpRecordSkill->wSkillMastery[cnt - 1] = sinSkill.UseSkill[cnt].UseSkillCount;
		else
			lpRecordSkill->wSkillMastery[cnt - 1] = 10000;

		lpRecordSkill->bShortKey[cnt - 1] = sinSkill.UseSkill[cnt].ShortKey | (sinSkill.UseSkill[cnt].MousePosi << 4);
	}
...
int	RestoreSkill(RECORD_SKILL* lpRecordSkill, DWORD dwLevelQuestLog)
{
	...
	/*if (lpRecordSkill->bSkillPoint[0] > 0 && lpRecordSkill->bSkillPoint[19] == 0) {
	...
	for (cnt = 0; cnt < SIN_MAX_USE_SKILL; cnt++) {
		if (cnt > 0) {
			sinSkill.UseSkill[cnt].Point = lpRecordSkill->bSkillPoint[cnt - 1];
			sinSkill.UseSkill[cnt].UseSkillCount = lpRecordSkill->wSkillMastery[cnt - 1];
			sinSkill.UseSkill[cnt].ShortKey = lpRecordSkill->bShortKey[cnt - 1] & 0xF;
			sinSkill.UseSkill[cnt].MousePosi = lpRecordSkill->bShortKey[cnt - 1] >> 4;
...
int	CheckSkillPoint(int Level, RECORD_SKILL* lpRecordSkill, int* spTotal, DWORD dwLevelQuestLog)
{
	...
	for (cnt = 1; cnt < 13; cnt++) {
		if (lpRecordSkill->bSkillPoint[cnt - 1] > 10) return FALSE;
		Point += abs(lpRecordSkill->bSkillPoint[cnt - 1]);
	}

	for (cnt = 13; cnt < SIN_MAX_USE_SKILL; cnt++) {
		if (lpRecordSkill->bSkillPoint[cnt - 1] > 10) return FALSE;
		EPoint += abs(lpRecordSkill->bSkillPoint[cnt - 1]);
	}
...
int rsRECORD_DBASE::ResotrRecordData(smCHAR_INFO* lpCharInfo, sITEM* lpItems, sITEM* lpItems2, sITEM* lpMouseItem)
{
	...
	for (cnt = 0; cnt < TransRecordData.ItemCount; cnt++) {
		...
		if (DeleteEventItem_TimeOut(&sRecordItem.sItemInfo) == TRUE) {
			...
			if (sRecordItem.sItemInfo.CODE == (sinOR2 | sin01) ||
				...
				sRecordItem.sItemInfo.CODE == (sinOR2 | sin28) || sRecordItem.sItemInfo.CODE == (sinOA1 | sin33) ||
				sRecordItem.sItemInfo.CODE == (sinOR2 | sin29) || sRecordItem.sItemInfo.CODE == (sinOA1 | sin34) ||
				sRecordItem.sItemInfo.CODE == (sinOR2 | sin30) || sRecordItem.sItemInfo.CODE == (sinOA1 | sin35) ||
				sRecordItem.sItemInfo.CODE == (sinOA1 | sin36) || sRecordItem.sItemInfo.CODE == (sinOA1 | sin37) ||
				sRecordItem.sItemInfo.CODE == (sinOR2 | sin33) ||

				sRecordItem.sItemInfo.CODE == (sinDA1 | sin31) || sRecordItem.sItemInfo.CODE == (sinDA2 | sin31) ||
	...
	if (TransRecordData.GameSaveInfo.Checksum != 0xBC)
	{
		ZeroMemory(&TransRecordData.GameSaveInfo.SkillInfo, sizeof(RECORD_SKILL));
	}
	...
	if (TransRecordData.GameSaveInfo.wForceOrbUsing[0] && TransRecordData.GameSaveInfo.wForceOrbUsing[1]) {
		...
		lpCurPlayer->dwForceOrbTime = (DWORD)TransRecordData.GameSaveInfo.wForceOrbUsing[1] * 1000;
	...
	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_HelpPet)
	{
		...
		cPCBANGPet.PetKind = TransRecordData.GameSaveInfo.dwParam_PrimeItem_HelpPet;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetHelpPetTimeTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(cPCBANGPet.PetKind + 7, chaPremiumitem.m_HelpPetTime, true, UpKeepItemName[cPCBANGPet.PetKind + 6], 500);
			cPCBANGPet.ShowPet();
		}
		else if (PrimeItem_Time < 0)
		{
	...
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_BigHead;
		if (PrimeItem_Time > 0)
		{
			BigHead = TransRecordData.GameSaveInfo.dwParam_PrimeItem_BigHead;
			chaPremiumitem.SetBigHeadPotionTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::BIGHEAD_POTION, chaPremiumitem.m_BigHeadPotionTime, true, UpKeepItemName[13], 30);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_VIP)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_VIP;
		if (PrimeItem_Time > 0)
		{
			PVIP = TransRecordData.GameSaveInfo.dwParam_PrimeItem_VIP;
			chaPremiumitem.SetVipTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::VIP, chaPremiumitem.m_VipTime, true, UpKeepItemName[13 + PVIP], 30);
		}
	}

	lpCurPlayer->PvP_Time = TransRecordData.GameSaveInfo.PvP_Time;
	lpCurPlayer->PvP_Param = TransRecordData.GameSaveInfo.PvP_Param;
...
int rsRECORD_DBASE::SendRecordDataToClient(rsPLAYINFO* lpPlayInfo, char* szName, smWINSOCK* lpsmSock, int Mode)
{
	...
	if (lpTransRecord->GameSaveInfo.PlayStageNum == QUEST_ARENA_FIELD) {
		if (lpTransRecord->smCharInfo.JOB_CODE <= 4 || lpTransRecord->smCharInfo.JOB_CODE != 9 || lpTransRecord->smCharInfo.JOB_CODE != 11)
...

{
	...
	if (lpTransRecord->Count >= lpTransRecord->Total - 1 && TransRecordData.code == smTRANSCODE_RECORDDATA) {
		TransRecordData.GameSaveInfo.SaveTime = tServerTime;
		...
		if (lpPlayInfo->AdminMode < 3 && exp > lpPlayInfo->spEXP_Out)
		{
			return -9;
		}

		if (lpPlayInfo->Checksum <= 0 && !lpPlayInfo->AdminMode)
		{
			TransRecordData.code = 0;
			return -9;
		}
...
int rsRecordMemoryBuff_CheckInvenItem(rsPLAYINFO* lpPlayInfo, int Mode)
{
	...
	if (lpPlayInfo->OpenWarehouseInfoFlag && (lpPlayInfo->dwDataError & rsDATA_ERROR_WAREHOUSE) == 0) {
		GetWareHouseFile(lpPlayInfo->szID, lpPlayInfo->WarehousePage, szFileName);
	...
	if (lpPlayInfo->OpenWarehouseInfoFlag) {
		...
		if (WareHouseFixFlag && !Mode) {
			if (SaveWareHouse(&WareHouseCheck, &TransWareHouse) == TRUE) {
				rsSaveWareHouseData(lpPlayInfo->szID, lpPlayInfo->WarehousePage, &TransWareHouse);
			}
		}
	}
	else {
			...
			if (fp) {
				...
				if (lstrcmpi(TransRecordData.smCharInfo.szName, sPlayUserData.szCharName[cnt]) == 0 &&
					...
					if (lpCharInfo->StartField == QUEST_ARENA_FIELD) {
						if (lpCharInfo->JOB_CODE <= 4 || lpCharInfo->JOB_CODE == 9 || lpCharInfo->JOB_CODE == 11)
...
int rsSaveWareHouseData(char* szID, int Page, TRANS_WAREHOUSE* lpTransWareHouse)
{
	char szFileName[128];
	FILE* fp;

	GetWareHouseFile(szID, Page, szFileName);
...
int rsLoadWareHouseData_Admin(rsPLAYINFO* lpPlayInfo, char* szID, int Day)
{
	...
	if (Day == 0)
		GetWareHouseFile(szID, lpPlayInfo->WarehousePage, szFileName);
	else
		GetWareHouseFile_Backup(szID, lpPlayInfo->WarehousePage, szFileName, Day);
...
int rsLoadServerForce(rsPLAYINFO* lpPlayInfo, sGAME_SAVE_INFO* lpGameSaveInfo)
{
	...
	if (lpGameSaveInfo->wForceOrbUsing[0] && lpGameSaveInfo->wForceOrbUsing[1])
	{
		...
		if (cnt >= 0 && cnt < 20)
		{
			if (lpGameSaveInfo->wForceOrbUsing[1] <= ForceOrbUseTime[cnt])
			{
				...
				lpPlayInfo->dwForceOrb_SaveTime = lpGameSaveInfo->wForceOrbUsing[1];
	...
	if (lpGameSaveInfo->dwTime_PrimeItem_HelpPet)
	{
		...
		lpPlayInfo->dwParam_PrimeItem_HelpPet = lpGameSaveInfo->dwParam_PrimeItem_HelpPet;
	}
	else
	{
		lpPlayInfo->dwTime_PrimeItem_HelpPet = 0;
		lpPlayInfo->dwParam_PrimeItem_HelpPet = 0;
		cPCBANGPet.ClosePet();
	}
	...
	{
		lpPlayInfo->dwTime_PrimeItem_BigHead = lpGameSaveInfo->dwTime_PrimeItem_BigHead;
		lpPlayInfo->dwParam_PrimeItem_BigHead = lpGameSaveInfo->dwParam_PrimeItem_BigHead;
	}
	else
	{
		lpPlayInfo->dwTime_PrimeItem_BigHead = 0;
		lpPlayInfo->dwParam_PrimeItem_BigHead = 0;
	}

	if (lpGameSaveInfo->dwTime_PrimeItem_VIP > (DWORD)tServerTime)
	{
		lpPlayInfo->dwTime_PrimeItem_VIP = lpGameSaveInfo->dwTime_PrimeItem_VIP;
		lpPlayInfo->dwParam_PrimeItem_VIP = lpGameSaveInfo->dwParam_PrimeItem_VIP;
	}
	else
	{
	...
	lpPlayInfo->QuestStart_State = lpGameSaveInfo->QuestStart_State;
	lpPlayInfo->QuestStart_Param1 = lpGameSaveInfo->QuestStart_Param1;
	lpPlayInfo->QuestStart_Param2 = lpGameSaveInfo->QuestStart_Param2;
	lpPlayInfo->QuestLv150A_State = lpGameSaveInfo->QuestLv150A_State;
	lpPlayInfo->QuestLv150A_Param = lpGameSaveInfo->QuestLv150A_Param;
	lpPlayInfo->QuestLv150A_Time = lpGameSaveInfo->QuestLv150A_Time;
	lpPlayInfo->QuestLv150B_State = lpGameSaveInfo->QuestLv150B_State;
	lpPlayInfo->QuestLv150B_Param = lpGameSaveInfo->QuestLv150B_Param;
	lpPlayInfo->QuestLv150B_Time = lpGameSaveInfo->QuestLv150B_Time;
	lpPlayInfo->PvP_Time = lpGameSaveInfo->PvP_Time;
	lpPlayInfo->PvP_Param = lpGameSaveInfo->PvP_Param;

	lpPlayInfo->WeaponClass = lpGameSaveInfo->WeaponClass;
	lpPlayInfo->ArmorClass = lpGameSaveInfo->ArmorClass;
	lpPlayInfo->ShieldClass = lpGameSaveInfo->ShieldClass;
	lpPlayInfo->BraceletsClass = lpGameSaveInfo->BraceletsClass;
	lpPlayInfo->GauntletsClass = lpGameSaveInfo->GauntletsClass;
	lpPlayInfo->BootsClass = lpGameSaveInfo->BootsClass;
	lpPlayInfo->RingClass = lpGameSaveInfo->RingClass;
	lpPlayInfo->AmyClass = lpGameSaveInfo->AmyClass;
	lpPlayInfo->ShelClass = lpGameSaveInfo->ShelClass;
	lpPlayInfo->TimeClass = lpGameSaveInfo->TimeClass;

	if (lpGameSaveInfo->WarehousePage_Time > (DWORD)tServerTime)
	{
		lpPlayInfo->WarehousePage_Time = lpGameSaveInfo->WarehousePage_Time;
	}
	else
	{
		lpPlayInfo->WarehousePage_Time = 0;
	}

	if (lpGameSaveInfo->RandomMonster_Time > (DWORD)tServerTime)
	{
		lpPlayInfo->RandomMonster_Time = lpGameSaveInfo->RandomMonster_Time;
		lpPlayInfo->RandomMonster_Param = lpGameSaveInfo->RandomMonster_Param;
	}
	else
	{
		lpPlayInfo->RandomMonster_Time = 0;
		lpPlayInfo->RandomMonster_Param = 0;
	}

	lpPlayInfo->QuestLv155A_State = lpGameSaveInfo->QuestLv155A_State;
	lpPlayInfo->QuestLv155A_Param = lpGameSaveInfo->QuestLv155A_Param;
	lpPlayInfo->QuestLv155A_Time = lpGameSaveInfo->QuestLv155A_Time;

	Quest::GetInstance()->SendProgress(lpPlayInfo);
...
int rsSaveServerForce(rsPLAYINFO* lpPlayInfo, sGAME_SAVE_INFO* lpGameSaveInfo)
{
	...
	if (lpGameSaveInfo->Checksum != 0xBC)
	{
		lpGameSaveInfo->Checksum = 0xBC;
	}

	if (lpPlayInfo->dwForceOrb_SaveTime) {
		lpGameSaveInfo->wForceOrbUsing[0] = (WORD)(lpPlayInfo->dwForceOrb_SaveCode & sinITEM_MASK3);
		lpGameSaveInfo->wForceOrbUsing[1] = lpPlayInfo->dwForceOrb_SaveTime;
	}
	else {
	...
	if (lpPlayInfo->dwTime_PrimeItem_ManaReduce)
	{
	...
	if (lpPlayInfo->dwTime_PrimeItem_MightofAwell)
	{
	...
	if (lpPlayInfo->dwTime_PrimeItem_MightofAwell2)
	{
	...
	if (lpPlayInfo->dwTime_PrimeItem_PhenixPet)
	{
	...
	if (lpPlayInfo->dwTime_PrimeItem_HelpPet)
	{
		lpGameSaveInfo->dwTime_PrimeItem_HelpPet = lpPlayInfo->dwTime_PrimeItem_HelpPet;
		lpGameSaveInfo->dwParam_PrimeItem_HelpPet = lpPlayInfo->dwParam_PrimeItem_HelpPet;
	}
	else
	{
		lpGameSaveInfo->dwTime_PrimeItem_HelpPet = 0;
		lpGameSaveInfo->dwParam_PrimeItem_HelpPet = 0;
		cPCBANGPet.ClosePet();
	}

	if (lpPlayInfo->dwTime_PrimeItem_StaminaReduce)
	{
	...
	if (lpPlayInfo->dwTime_PrimeItem_BigHead)
	{
		lpGameSaveInfo->dwTime_PrimeItem_BigHead = lpPlayInfo->dwTime_PrimeItem_BigHead;
		lpGameSaveInfo->dwParam_PrimeItem_BigHead = lpPlayInfo->dwParam_PrimeItem_BigHead;
	}
	else
	{
		lpGameSaveInfo->dwTime_PrimeItem_BigHead = 0;
		lpGameSaveInfo->dwParam_PrimeItem_BigHead = 0;
	}

	if (lpPlayInfo->dwTime_PrimeItem_VIP > (DWORD)tServerTime)
	{
		lpGameSaveInfo->dwTime_PrimeItem_VIP = lpPlayInfo->dwTime_PrimeItem_VIP;
		lpGameSaveInfo->dwParam_PrimeItem_VIP = lpPlayInfo->dwParam_PrimeItem_VIP;
	}
	else
	{
		lpGameSaveInfo->dwTime_PrimeItem_VIP = 0;
		lpGameSaveInfo->dwParam_PrimeItem_VIP = 0;
	}

	lpGameSaveInfo->QuestStart_State = lpPlayInfo->QuestStart_State;
	lpGameSaveInfo->QuestStart_Param1 = lpPlayInfo->QuestStart_Param1;
	lpGameSaveInfo->QuestStart_Param2 = lpPlayInfo->QuestStart_Param2;

	lpGameSaveInfo->QuestLv150A_State = lpPlayInfo->QuestLv150A_State;
	lpGameSaveInfo->QuestLv150A_Param = lpPlayInfo->QuestLv150A_Param;
	lpGameSaveInfo->QuestLv150A_Time = lpPlayInfo->QuestLv150A_Time;
	lpGameSaveInfo->QuestLv150B_State = lpPlayInfo->QuestLv150B_State;
	lpGameSaveInfo->QuestLv150B_Param = lpPlayInfo->QuestLv150B_Param;
	lpGameSaveInfo->PvP_Time = lpPlayInfo->PvP_Time;
	lpGameSaveInfo->PvP_Param = lpPlayInfo->PvP_Param;

	lpGameSaveInfo->WeaponClass = lpPlayInfo->WeaponClass;
	lpGameSaveInfo->ArmorClass = lpPlayInfo->ArmorClass;
	lpGameSaveInfo->ShieldClass = lpPlayInfo->ShieldClass;
	lpGameSaveInfo->BraceletsClass = lpPlayInfo->BraceletsClass;
	lpGameSaveInfo->GauntletsClass = lpPlayInfo->GauntletsClass;
	lpGameSaveInfo->BootsClass = lpPlayInfo->BootsClass;
	lpGameSaveInfo->RingClass = lpPlayInfo->RingClass;
	lpGameSaveInfo->AmyClass = lpPlayInfo->AmyClass;
	lpGameSaveInfo->ShelClass = lpPlayInfo->ShelClass;

	if (lpPlayInfo->TimeClass > (DWORD)tServerTime)
		lpGameSaveInfo->TimeClass = lpPlayInfo->TimeClass;
	else
		lpGameSaveInfo->TimeClass = 0;

	if (lpPlayInfo->WarehousePage_Time > (DWORD)tServerTime)
		lpGameSaveInfo->WarehousePage_Time = lpPlayInfo->WarehousePage_Time;
	else
		lpGameSaveInfo->WarehousePage_Time = 0;

	lpGameSaveInfo->QuestLv155A_State = lpPlayInfo->QuestLv155A_State;
	lpGameSaveInfo->QuestLv155A_Param = lpPlayInfo->QuestLv155A_Param;
	lpGameSaveInfo->QuestLv155A_Time = lpPlayInfo->QuestLv155A_Time;

	if (lpPlayInfo->RandomMonster_Time > (DWORD)tServerTime)
	{
		lpGameSaveInfo->RandomMonster_Time = lpPlayInfo->RandomMonster_Time;
		lpGameSaveInfo->RandomMonster_Param = lpPlayInfo->RandomMonster_Param;
	}
	else
	{
		lpGameSaveInfo->RandomMonster_Param = 0;
		lpGameSaveInfo->RandomMonster_Time = 0;
	}
...
int	rsLoadPostBox(rsPLAYINFO* lpPlayInfo)
{
	...
	if (lpPlayInfo->lpPostBoxItem) delete lpPlayInfo->lpPostBoxItem;
...
int rsAddPostBoxItem(rsPLAYINFO* pcUnitData, const char* ItemCode, int Spec, char* pszMessage, bool bSendToCharacter)
{
	if (!pcUnitData->szID[0])
		return FALSE;

	char szFileName[64];
	GetPostBoxFile(pcUnitData->szID, szFileName);

	FILE* pFile = NULL;
	fopen_s(&pFile, szFileName, "a+");
	if (pFile)
	{
		fprintf(pFile, "%s		%s		%d		\"%s\"\r\n", bSendToCharacter ? pcUnitData->smCharInfo.szName : "*", ItemCode, Spec, pszMessage);
		fclose(pFile);
