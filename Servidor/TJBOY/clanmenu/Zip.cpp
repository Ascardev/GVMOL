...
BOOL CZip::ZipCompression(char* data,char* FileName,DWORD len)
{
	...
	ulLen = (unsigned long)len;

	
	
	ZeroMemory(&m_sZi,sizeof(ZIPINFO));
	m_sZi.dwFileSize = m_dwFileSize;				
	m_sZi.dwVersion = YULVERSION;					
	strcpy(m_sZi.szZipVersion,ZLIB_VERSION);		
	strcpy(m_sZi.cUnCompFilName,FileName);			
