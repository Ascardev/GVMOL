...
int	rsPLAYINFO::UpdatePartyPlayInfo()
{
	...
	TransPartyPlayInfo.PlayInfo[cnt2].JobCode = smCharInfo.JOB_CODE;
...
int	PartyExpTable[8][2] = {
	{ 1.f, 0 },       // 1 Player = 100%
	{ 0.70f, 0.70f }, // 2 Player = 130%
	{ 0.60f, 0.60f }, // 3 Player = 150%
	{ 0.55f, 0.55f }, // 4 Player = 180%
	{ 0.52f, 0.52f }, // 5 Player = 200%
	{ 0.50f, 0.50f }, // 6 Player = 222%
	{ 0.22f, 0.21f },
	{ 0.20f, 0.20f }
};
...
void LoadPartyExpTable()
{
	File exp(".\\config.ini");
	for (int i = 1; i < 6; i++)
	{
		char buff[255];
		wsprintf(buff, "%d Membros", i + 1);
		PartyExpTable[i][0] = (float)exp.readInt("PartyRate", buff) / 100;
		PartyExpTable[i][1] = (float)exp.readInt("PartyRate", buff) / 100;
	}
}
...
int rsPLAYINFO::GetPartyExp(smCHAR* lpChar, rsPLAYINFO* lpPlayInfo)
{
	...
	Exp = srGetTotalExp(lpChar->smMonsterInfo.GetExp, lpChar->smCharInfo.Level - lpPartyPlayers[cnt]->smCharInfo.Level, lpPartyPlayers[cnt]);
	Exp += (Exp * ((PartyUserCount - 1) * 5)) / 100;

	PartyExp[0] = Exp;			//본인 획득
	PartyExp[1] = Exp;			//파티원 획득
	...
	//	if (lpPartyPlayers[cnt] && lpPartyPlayers[cnt]->lpsmSock)
	//	{
	//		for (int cnt2 = 0; cnt2 < PARTY_PLAYER_MAX; cnt2++)
	//		{
	//			if (lpPartyPlayers[cnt2] && lpPartyPlayers[cnt2]->lpsmSock)
	//			{
	//				if (lstrcmp(lpPartyPlayers[cnt]->szName, lpPartyPlayers[cnt2]->szName) != 0 && lstrcmp(lpPartyPlayers[cnt]->lpsmSock->szIPAddr, lpPartyPlayers[cnt2]->lpsmSock->szIPAddr) == 0)
	//				{
	//					Quest++;
	//				}
	//			}
	...
	for (cnt = 0; cnt < PARTY_PLAYER_MAX; cnt++)
	{
		if (lpPartyPlayers[cnt] && lpPartyPlayers[cnt]->lpsmSock && lpPartyPlayers[cnt]->Position.Area == lpPlayInfo->Position.Area)
		{
			if (lpPartyPlayers[cnt] == lpPlayInfo)
			{
				smTransCommand.code = smTRANSCODE_ADDEXP;
				smTransCommand.WParam = PartyExp[0];
				Quest::GetInstance()->UpdaterProgressOnServer(lpPartyPlayers[cnt], lpChar, smTRANSCODE_ADDEXP);
				Sucess++;
			}
			else
			{
				...
				if (dist < PARTY_GETTING_DIST)
				{
					...
					if (lpPlayInfo->lpsmSock)
					{
						/*if (Quest == 0)
						{*/
						Quest::GetInstance()->UpdaterProgressOnServer(lpPartyPlayers[cnt], lpChar, smTRANSCODE_ADDEXP_PARTY);
						/*}
						else smTransCommand.code = 0;*/
					}
				}
				else
			...
			if (lpPartyPlayers[cnt]->dwTime_PrimeItem_ExpUp)
			{
...
#define	CHK_SERVER_MONEY_MAX2	550000000
#define	CHK_SERVER_MONEY_MIN	-550000000
