...
int rsRecvAttackData_Old(rsPLAYINFO* lpPlayInfo, TRANS_ATTACKDATA* lpTransAttackData, smCHAR* lpTargetChar, int Type)
{
	...
	if ((lpTransAttackData->Power >= rsServerConfig.LimitDamage + 4000 && (lpPlayInfo->smCharInfo.JOB_CODE == 4 || lpPlayInfo->smCharInfo.JOB_CODE == 5)) || lpTransAttackData->Power >= rsServerConfig.LimitDamage) {
	...
	if (lpChar) {
		...
		if (lpChar->MotionInfo->State != CHRMOTION_STATE_DEAD && lpChar->MotionInfo->State != CHRMOTION_STATE_WARP) {
			...
			if (lpChar->smCharInfo.State) {
				...
				if (cnt > 0) {
					...
					if (lpPlayInfo->dwSkill_HolyBenedic_Time) {
						if (lpPlayInfo->dwSkill_HolyBenedic_Time > dwPlayServTime) {
							if (lpChar->smCharInfo.Brood == smCHAR_MONSTER_UNDEAD)
								cnt += (cnt * lpPlayInfo->dwSkill_HolyBenedic_Param) / 100;
						}
						else {
							lpPlayInfo->dwSkill_HolyBenedic_Time = 0;
							lpPlayInfo->dwSkill_HolyBenedic_Param = 0;
						}
					}
				...
				if (abs((double)lpPlayInfo->dwLastAttackTime - dwTime) > 1500) {
					lpPlayInfo->dwLastAttackTime = dwTime;
					...
					if (lpChar->smMonsterInfo.EventCode == 1001)
					{
						smTRANS_COMMAND sPacket;
						sPacket.code = smTRANSCODE_cMIXGUIDE;
					...
					if (lpChar->smMonsterInfo.SellAttackItemCount ||
						...
						if (lpChar->smMonsterInfo.SellAttackItemCount)
							memcpy(&lpPlayInfo->SellAttackItem, &lpChar->smMonsterInfo.SellAttackItem, sizeof(lpPlayInfo->SellAttackItem));
						else
							memset(&lpPlayInfo->SellAttackItem, 0, sizeof(lpPlayInfo->SellAttackItem));

						if (lpChar->smMonsterInfo.SellDefenceItemCount)
							memcpy(&lpPlayInfo->SellDefenceItem, &lpChar->smMonsterInfo.SellDefenceItem, sizeof(lpPlayInfo->SellDefenceItem));
						else
							memset(&lpPlayInfo->SellDefenceItem, 0, sizeof(lpPlayInfo->SellDefenceItem));

						if (lpChar->smMonsterInfo.SellEtcItemCount)
							memcpy(&lpPlayInfo->SellEtcItem, &lpChar->smMonsterInfo.SellEtcItem, sizeof(lpPlayInfo->SellEtcItem));
						else
							memset(&lpPlayInfo->SellEtcItem, 0, sizeof(lpPlayInfo->SellEtcItem));
					...
					if (lpChar->smMonsterInfo.CollectMoney) {
						SendOpenCollectMoney(lpsmSock, 0);
					}
#ifdef _LANGUAGE_VEITNAM
					...
					if (lpChar->smMonsterInfo.Remodel)
					{
						SendOpenRemodelItem(lpsmSock);
					}

					if (lpChar->smMonsterInfo.CoinShop)
					{
						SendOpenCoinShop(lpsmSock);
						CoinShop::GetInstance()->SendCoin(lpPlayInfo);
						CoinShop::GetInstance()->SendList(lpPlayInfo);
					}

					if (lpChar->smMonsterInfo.TimeShop)
					{
						SendOpenTimeShop(lpsmSock);
						TimeShop::GetInstance()->SendList(lpPlayInfo);
					}

					if (lpChar->smMonsterInfo.AwardItem)
					{
						AwardItem::GetInstance()->SendList(lpPlayInfo);
					}

					if (lpChar->smMonsterInfo.Roulette)
					{
						Roulette::GetInstance()->SendList(lpPlayInfo);
					}
				}
			}
		...
		if (lpPlayInfo2)
		{
			if (rsServerConfig.Enable_PK || lpPlayInfo->smCharInfo.State != lpPlayInfo2->smCharInfo.State || (lpPlayInfo->PvP_Param && lpPlayInfo2->PvP_Param) || lpPlayInfo->Position.Area == 43 || lpPlayInfo->Position.Area == 50)
			{
...
int rsSendAttackUser(rsPLAYINFO* lpPlayInfo, rsPLAYINFO* lpPlayInfo2, int Power, int AttackState, short sParam1, short sParam2)
{
	...
	smTRANS_COMMAND		smTransCommand;
	int	temp;
	int pvp = lpPlayInfo->PvP_Param && lpPlayInfo2->PvP_Param;

	if (!lpPlayInfo->AdminMode && !pvp && lpPlayInfo->Position.Area != 43 && lpPlayInfo->Position.Area != 50)
	{
	...
	if (lpPlayInfo2->dwSkill_RainMaker_Time) {
		if (lpPlayInfo2->dwSkill_RainMaker_Time > dwPlayServTime) {
			TransAttackData.Power -= lpPlayInfo2->dwSkill_RainMaker_Param;
			if (TransAttackData.Power < 0) return FALSE;
		}
		else {
	...
	if (lpPlayInfo2->dwSkill_I_Bulkup_Time) {
		if (lpPlayInfo2->dwSkill_I_Bulkup_Time > dwPlayServTime) {
			TransAttackData.Power -= lpPlayInfo->dwSkill_I_Bulkup_Param;
			if (TransAttackData.Power < 0) return FALSE;
		}
		else {
			lpPlayInfo2->dwSkill_I_Bulkup_Time = 0;
			lpPlayInfo2->dwSkill_I_Bulkup_Param = 0;
		}
	}
	...
	if (lpPlayInfo2->dwSkill_Krishna_Time) {
		if (lpPlayInfo2->dwSkill_Krishna_Time > dwPlayServTime) {
			TransAttackData.Power -= Krishna_Abs[lpPlayInfo2->dwSkill_Krishna_Param];
			if (TransAttackData.Power < 0) return FALSE;
		}
		else {
	...
	if (lpPlayInfo2->dwSkill_Tenus_Time) {
		if (lpPlayInfo2->dwSkill_Tenus_Time > dwPlayServTime) {
			TransAttackData.Power += (TransAttackData.Power * lpPlayInfo2->dwSkill_Tenus_Param) / 100;
			if (TransAttackData.Power < 0) return FALSE;
		}
		else {
			lpPlayInfo2->dwSkill_Tenus_Time = 0;
			lpPlayInfo2->dwSkill_Tenus_Param = 0;
		}
	}
	...
	if (lpPlayInfo2->dwSkill_B_Berserker_Time) {
		if (lpPlayInfo2->dwSkill_B_Berserker_Time > dwPlayServTime) {
			TransAttackData.Power -= B_Berserker_SubAbsorb[lpPlayInfo2->dwSkill_B_Berserker_Param];
			if (TransAttackData.Power < 0) return FALSE;
		}
		else {
			lpPlayInfo2->dwSkill_B_Berserker_Time = 0;
			lpPlayInfo2->dwSkill_B_Berserker_Param = 0;
		}
	}

	if (lpPlayInfo2->dwSkill_Amplified_Time) {
		if (lpPlayInfo2->dwSkill_Amplified_Time > dwPlayServTime) {
			TransAttackData.Power -= Amplified_SubAbsorb[lpPlayInfo2->dwSkill_Amplified_Param];
			if (TransAttackData.Power < 0) return FALSE;
		}
		else {
	...
	if (lpPlayInfo2 && lpPlayInfo2->dwSkill_Alas_Time)
	{
		if (lpPlayInfo2->dwSkill_Alas_Time > dwPlayServTime)
		{
			if ((rand() % 100) < Alas_EvasionPercent[lpPlayInfo2->dwSkill_Alas_Param])
			{
				...
				if (lpPlayInfo2->AdminMode > 2)
				{
					wsprintf(szDispDamage, "> %s Evasão adicional ( Alas [%d] )", lpPlayInfo->smCharInfo.szName, Alas_EvasionPercent[lpPlayInfo2->dwSkill_Alas_Param]);
					rsDisplayDamgeToClient(lpPlayInfo2, szDispDamage);
				}
#endif
				...
				smTransCommand.size = sizeof(smTRANS_COMMAND);
				smTransCommand.WParam = 0;
				smTransCommand.LParam = SKILL_PLAY_ALAS;
				smTransCommand.SParam = lpPlayInfo->dwObjectSerial;
				smTransCommand.EParam = 0;
				lpPlayInfo2->lpsmSock->Send((char*)&smTransCommand, smTransCommand.size, TRUE);
	...
	if (lpPlayInfo2->dwSkill_SummonRamiel_Time) {
		if (lpPlayInfo2->dwSkill_SummonRamiel_Time > dwPlayServTime) {
			if ((rand() % 100) < Summon_Ramiel_BlockPercent[lpPlayInfo2->dwSkill_SummonRamiel_Param]) {
#ifdef DISP_DAMAGE
				if (lpPlayInfo2->AdminMode > 2) {
					wsprintf(szDispDamage, ">%s °ø°Ý Ãß°¡ È¸ÇÇ ( Summon Ramiel [%d] )", lpPlayInfo->smCharInfo.szName, Summon_Ramiel_BlockPercent[lpPlayInfo2->dwSkill_SummonRamiel_Param]);
					rsDisplayDamgeToClient(lpPlayInfo2, szDispDamage);
				}
#endif
			...
			lpPlayInfo2->dwSkill_SummonRamiel_Time = 0;
			lpPlayInfo2->dwSkill_SummonRamiel_Param = 0;
		}
	}
...
int RecvRangeAttack_User(rsPLAYINFO* lpPlayInfo, TRANS_SKIL_ATTACKDATA* lpTransSkilAttackData, int AreaNum, int PlaySkillCode)
{
			...
			if (lpCharInfo && lpCharInfo->lpsmSock && lpCharInfo->smCharInfo.State)
			{
				for (cnt = 0; cnt < lpTransSkilAttackData->TargetCount; cnt++)
				{
					if (lpCharInfo->dwObjectSerial == lpTransSkilAttackData->dwTarObjectSerial[cnt])
					{
						...
						else if (lpTransSkilAttackData->AttackState == 2)
						{
	...
	case 3:
		...
		else if (SkillCode == SKILL_PLAY_SWORD_OF_JUSTICE) {
			time = 8;
			SlowSpeed = 200;
		}
		else if (SkillCode == SKILL_PLAY_C_TRAP) {
			time = 8;
			SlowSpeed = 200;
		}
		else if (SkillCode == SKILL_PLAY_GROUND_PIKE) {
			time = 10;
			SlowSpeed = 128;
...
int RecvRangeAttack(rsPLAYINFO* lpPlayInfo, TRANS_SKIL_ATTACKDATA* lpTransSkilAttackData, int AreaNum, int PlaySkillCode)
{
	...
	case 3:
		if (SkillCode == SKILL_PLAY_GLACIAL_SPIKE) {
			...
			cnt2 = lpTransSkilAttackData->AttackSize >> FLOATNS;
			for (cnt = 0; cnt < 10; cnt++) {
				if (C_Trap_Range[cnt] == cnt2) {
					time = C_Trap_Delay1[cnt] * 17;
					break;
				}
			}
		}
		else if (SkillCode == SKILL_GROUND_PIKE) {
			time = 175;
			SlowSpeed = 0;
			cnt2 = lpTransSkilAttackData->AttackSize >> FLOATNS;
			for (cnt = 0; cnt < 10; cnt++) {
			...
			if (lpChar && lpChar->Flag && lpChar->smCharInfo.State && lpChar->MotionInfo->State != CHRMOTION_STATE_DEAD) {
				for (cnt = 0; cnt < lpTransSkilAttackData->TargetCount; cnt++) {
					if (lpChar->dwObjectSerial == lpTransSkilAttackData->dwTarObjectSerial[cnt])
					{
						...
						lpChar->PlayWarCry[0] = Warcry_Reduce[SkillPoint];
						lpChar->PlayWarCry[1] = Warcry_Time[SkillPoint];

						lpChar->WarCry_Time = dwPlayServTime + Warcry_Time[SkillPoint] * 1000;
						lpChar->WarCry_Param = Warcry_Damage[SkillPoint];
...
int RecvRangeAttackUser(rsPLAYINFO* lpPlayInfo, TRANS_SKIL_ATTACKDATA* lpTransSkilAttackData)
{
	...
	if (!lpPlayInfo->AdminMode && !lpPlayInfo->PvP_Param && lpPlayInfo->Position.Area != 43 && lpPlayInfo->Position.Area != 50) {
...
int rsSetSkillAttack(rsPLAYINFO* lpPlayInfo, smCHAR* lpChar, TRANS_ATTACKDATA* lpTransAttackData, TRANS_ATTACKDATA2* lpTransAttackData2)
{
	...
	case SKILL_PLAY_MULTI_SPARK:
		...
		lpTransAttackData->Power += GetRandomPos(lpTransAttackData2->Power[0], lpTransAttackData2->Power[1]);
		lpTransAttackData->Power += (lpTransAttackData->Power * M_Spark_Damage[Point]) / 100;
		lpTransAttackData2->Critical[0] = 0;
		break;
	...
	case SKILL_PLAY_FIRE_BOLT:					//ÆÄÀÌ¾î º¼Æ®
		...
		lpTransAttackData->Power += pow;
		lpTransAttackData->Power += (pow * FireBolt_Damage[Point]) / 100;
	...
	case SKILL_PLAY_DEAD_RAY:
		...
		pow = GetRandomPos(lpTransAttackData2->Power[0], lpTransAttackData2->Power[1]);
		lpTransAttackData->Power += pow;
		lpTransAttackData->Power += (pow * Dead_Ray_Damage[Point]) / 100;
	...
	case SKILL_PLAY_COLUMN_OF_WATER:
		...
		pow = GetRandomPos(lpTransAttackData2->Power[0], lpTransAttackData2->Power[1]);
		lpTransAttackData->Power += pow;
		lpTransAttackData->Power += (pow * Watornado_Damage[Point]) / 100;
	...
	case SKILL_PLAY_VIGOR_BALL:
		...
		if (lpTransAttackData2->Power[0] > lpTransAttackData2->Power2[0] && lpTransAttackData2->Power[1] > lpTransAttackData2->Power2[1])
		{
			lpTransAttackData->Power += GetRandomPos(lpTransAttackData2->Power2[0], lpTransAttackData2->Power2[1]);
	...
	case SKILL_PLAY_PET_ATTACK2:
		...
		if (!lpPlayInfo->dwTime_PrimeItem_HelpPet || (dwPlayServTime - lpPlayInfo->dwPCBang_Pet_AttackTime) < 1000) return FALSE;
	...
	case SKILL_PLAY_RAMIEL:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_PRIESTESS) {
			if (!lpPlayInfo->dwSkill_SummonRamiel_Time || lpPlayInfo->dwSkill_SummonRamiel_Time < dwPlayServTime) {
				lpPlayInfo->dwSkill_SummonRamiel_Time = 0;
				return FALSE;
			}

			pow1 = Summon_Ramiel_Damage[lpPlayInfo->dwSkill_SummonRamiel_Param][0];
			pow2 = Summon_Ramiel_Damage[lpPlayInfo->dwSkill_SummonRamiel_Param][1];
	...
	case SKILL_PLAY_B_UP:
		...
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * BeatUp_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_LOW_KICK:
		...
		{
			pow = GetRandomPos(LowKick_Damage[Point - 1][0], LowKick_Damage[Point][1]);
			lpTransAttackData->Power += pow;
			break;
		}
	...
	case SKILL_PLAY_DBBLOW:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_MARTIAL)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * DBBlow_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_H_STRAIGHT:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_MARTIAL)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * Straight_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_PATRIOT:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_MARTIAL)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * Patriot_Damage[Point]) / 100;
			lpTransAttackData2->Critical[0] += (lpTransAttackData2->Critical[0] * Patriot_Critical[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_T_CANNON:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_MARTIAL)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * Cannon_Damage[Point]) / 100;
			break;
		}
	case SKILL_PLAY_J_HEELKICK:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_MARTIAL)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * HeelKick_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_COMBINATION:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_MARTIAL)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * Combination_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_B_CHECK:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_MARTIAL)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * Check_Damage[Point]) / 100;
			lpTransAttackData2->Critical[0] += (lpTransAttackData2->Critical[0] * Check_Critical[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_TYPHOON:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_MARTIAL)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * Typhoon_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_D_HIT:
		...
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * D_Hit_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_M_BLOW:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_FIGHTER)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * M_Blow_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_P_DASH:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_FIGHTER)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * P_Dash_Damage[Point]) / 100;
			break;
		}

	case SKILL_PLAY_R_SMASH:
		if (lpPlayInfo->smCharInfo.JOB_CODE != JOBCODE_MECHANICIAN) return FALSE;
		lpTransAttackData->Power += (lpTransAttackData->Power * R_Smash_Damage[Point]) / 100;
		break;

	case SKILL_PLAY_E_SHOT:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_ARCHER)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * E_Shot_Damage[Point]) / 100;
			lpTransAttackData->AttackState = 5;
			break;
		}
	...
	case SKILL_PLAY_S_ROPE:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_ARCHER)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * S_Rope_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_N_SPLASH:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_ARCHER)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * N_Splash_Damage[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_D_REAPER:
		...
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * D_Reaper_AddAtk[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_SS_ATTACK:
		if (lpPlayInfo->smCharInfo.JOB_CODE == JOBCODE_PIKEMAN)
		{
			lpTransAttackData->Power += (lpTransAttackData->Power * SS_Attack_Rating[Point]) / 100;
			break;
		}
	...
	case SKILL_PLAY_G_COUP:
		...
		lpTransAttackData->Power += (lpTransAttackData->Power * G_Coup_Damage[Point]) / 100;
		break;

	case SKILL_PLAY_S_FEAR:
		if (lpPlayInfo->smCharInfo.JOB_CODE != JOBCODE_ATALANTA) return FALSE;

		lpTransAttackData->Power += (lpTransAttackData->Power * S_Fear_DamagePercent[Point]) / 100;
		break;
	...
	case SKILL_PLAY_SILRAPHIM:
		...
		lpTransAttackData->Power += (lpTransAttackData->Power * Silraphim_Damage[Point]) / 100;
	...
	case SKILL_PLAY_IGNIS:
		...
		lpTransAttackData->Power += (lpTransAttackData->Power * Ignis_Damage[Point]) / 100;
	...
	case SKILL_PLAY_TENUS:
		...
		lpPlayInfo->dwSkill_Tenus_Time = dwPlayServTime + Tenus_Time[Point] * 1000;
		lpPlayInfo->dwSkill_Tenus_Param = Tenus_Damage[Point];

		//if (lpChar)
		//{
		//	lpChar->smCharInfo.Life[0] -= (lpChar->smCharInfo.Life[0] * Tenus_HP[Point]) / 100;
		//	lpChar->dwTenusTime = Tenus_Time[Point] * 17;
		//}
	...
	case SKILL_PLAY_S_IMPACT:
		...
		lpTransAttackData->Power += (lpTransAttackData->Power * S_Impact_Damage[Point]) / 100;
	...
	case SKILL_PLAY_P_ICE:
		...
		lpTransAttackData->Power += (lpTransAttackData->Power * P_Ice_Damage[Point]) / 100;
...
int rsSetSkillPlus(rsPLAYINFO* lpPlayInfo, smCHAR* lpChar, TRANS_ATTACKDATA* lpTransAttackData, TRANS_ATTACKDATA2* lpTransAttackData2)
{
	...
	{
		if (lpPlayInfo->dwSkill_Blind_Time < dwPlayServTime)
		{
		...
		lpTransAttackData->Power += (lpTransAttackData->Power * Vanish_Damage[lpPlayInfo->dwSkill_Blind_Param]) / 100;
	...
	if (lpPlayInfo->dwSkill_P_Enhence_Time) {
		if (lpPlayInfo->dwSkill_P_Enhence_Time < dwPlayServTime) {
			lpPlayInfo->dwSkill_P_Enhence_Time = 0;
			lpPlayInfo->dwSkill_P_Enhence_Param = 0;
			return TRUE;
		}

		lpTransAttackData->Power += P_Enhence_Damage[1][lpPlayInfo->dwSkill_P_Enhence_Param];
	}
	...
	if (lpPlayInfo->dwSkill_B_Berserker_Time) {
		...
		lpTransAttackData->Power += B_Berserker_AddAttack[lpPlayInfo->dwSkill_B_Berserker_Param];
	}

	if (lpPlayInfo->dwSkill_Anima_Time) {
		if (lpPlayInfo->dwSkill_Anima_Time < dwPlayServTime) {
			lpPlayInfo->dwSkill_Anima_Time = 0;
			lpPlayInfo->dwSkill_Anima_Param = 0;
			return TRUE;
		}

		lpTransAttackData->Power += (lpTransAttackData->Power * lpPlayInfo->dwSkill_Anima_Param) / 100;
	}
	...
	if (lpPlayInfo->dwSkill_Amplified_Time)
	{
		...
		lpTransAttackData->Power += Amplified_AddAttack[lpPlayInfo->dwSkill_Amplified_Param];
	}
...
int rsSetSkillRangeAttack(rsPLAYINFO* lpPlayInfo, smCHAR* lpChar, TRANS_ATTACKDATA* lpTransAttackData, TRANS_SKIL_ATTACKDATA* lpTransSkillAttackData, TRANS_SKIL_ATTACKDATA2* lpTransRangeAttack)
{
	...
	case SKILL_PLAY_WAR_CRY:
		if (lpPlayInfo->smCharInfo.JOB_CODE != JOBCODE_MARTIAL) return FALSE;

		lpTransSkillAttackData->AttackSize = Warcry_Range[Point];
		lpTransSkillAttackData->Power = Warcry_Time[Point];
		lpTransSkillAttackData->AttackState = 8;
	...
	case SKILL_PLAY_E_SHOT:
		...
		if (lpPlayInfo->smCharInfo.JOB_CODE != JOBCODE_ARCHER) return FALSE;
		lpTransSkillAttackData->Power = GetRandomPos(E_Shot_Damage[Point], E_Shot_Damage[Point] + (lpPlayInfo->smCharInfo.Level / 5));
		lpTransSkillAttackData->AttackState = 5;
		return TRUE;
	...
	case SKILL_PLAY_P_DASH:
		...
		lpTransSkillAttackData->Power += (lpTransSkillAttackData->Power * P_Dash_Damage[Point]) / 100;
		lpTransSkillAttackData->AttackSize = P_Dash_Range[Point];
		lpTransSkillAttackData->AttackState = 0;
	...
	case SKILL_PLAY_GLACIAL_SPIKE:
		...
		if (lpChar && lpChar->smCharInfo.Brood != smCHAR_MONSTER_USER)
		{
			lpTransAttackData->Power *= 3;
		}
	...
	case SKILL_PLAY_FLAME_WAVE:
		if (lpPlayInfo->smCharInfo.JOB_CODE != JOBCODE_MAGICIAN) return FALSE;
		lpTransSkillAttackData->Power += GetRandomPos(lpTransRangeAttack->Power[0], lpTransRangeAttack->Power[1]);
		lpTransSkillAttackData->Power += (lpTransSkillAttackData->Power * Flame_Wave_FireDamage[Point]) / 100;
		lpTransSkillAttackData->AttackSize = Flame_Wave_Area[Point];
		lpTransSkillAttackData->AttackState = 100;
		return TRUE;
	...
	case SKILL_PLAY_POLLUTED:
		if (lpPlayInfo->smCharInfo.JOB_CODE != JOBCODE_ASSASSINE) return FALSE;

		if (lpTransSkillAttackData->TargetCount > Polluted_AttackNum[Point]) return FALSE;
	...
	case SKILL_PLAY_R_ELBOW:
		...
		lpTransSkillAttackData->Power += GetRandomPos(lpTransRangeAttack->Power[0], lpTransRangeAttack->Power[1]);
		lpTransSkillAttackData->Power += (lpTransSkillAttackData->Power * Elbow_Damage[Point]) / 100;
		lpTransSkillAttackData->AttackSize = 340;
		lpTransSkillAttackData->AttackState = 101;
		return TRUE;
	...
	case SKILL_PLAY_M_BLOW:
		...
		lpTransAttackData->Power = GetRandomPos(lpTransRangeAttack->Power[0], lpTransRangeAttack->Power[1]);
		lpTransAttackData->Power += (lpTransAttackData->Power * M_Blow_Damage[Point]) / 100;
		lpTransSkillAttackData->Power = GetRandomPos(M_Blow_RangeDamage[Point][0], M_Blow_RangeDamage[Point][1]);
		lpTransSkillAttackData->AttackState = 0;
		return TRUE;

	case SKILL_PLAY_F_SPEAR:
		if (lpPlayInfo->smCharInfo.JOB_CODE != JOBCODE_PIKEMAN) return FALSE;
		if (lpTransSkillAttackData->TargetCount > F_Spear_Num[Point]) return FALSE;

		lpTransSkillAttackData->AttackSize = F_Spear_Area[Point];
		lpTransSkillAttackData->Power = GetRandomPos(F_Spear_Damage[Point][0], F_Spear_Damage[Point][1]);

		return TRUE;

	case SKILL_PLAY_LAND_M:
		if (lpPlayInfo->smCharInfo.JOB_CODE != JOBCODE_MECHANICIAN) return FALSE;
		if (lpTransSkillAttackData->TargetCount > Land_M_AttackNum[Point] + 1) return FALSE;

		lpTransAttackData->Power = GetRandomPos(lpTransRangeAttack->Power[0], lpTransRangeAttack->Power[1]);
		lpTransAttackData->Power += (lpTransAttackData->Power * Land_M_DamagePercent[Point]) / 100;
		lpTransSkillAttackData->AttackSize = Land_M_Area[Point];
		lpTransSkillAttackData->Power = GetRandomPos(Land_M_AreaDamage[Point][0], Land_M_AreaDamage[Point][1]);
		lpTransSkillAttackData->AttackState = 0;
		dm_Critical_Temp = lpTransRangeAttack->Critical[0];
		return TRUE;
	...
	case SKILL_PLAY_C_MOON:
		...
		lpTransAttackData->Power = GetRandomPos(lpTransRangeAttack->Power[0], lpTransRangeAttack->Power[1]);
		lpTransAttackData->Power += (lpTransAttackData->Power * C_Moon_Damage[Point]) / 100;
		lpTransSkillAttackData->AttackSize = C_Moon_Range[Point];
		lpTransSkillAttackData->AttackState = 0;
		return TRUE;
	...
	case SKILL_PLAY_C_TRAP:
		...
		lpTransAttackData->Power = GetRandomPos(lpTransRangeAttack->Power[0], lpTransRangeAttack->Power[1]);
		lpTransAttackData->Power += (lpTransAttackData->Power * C_Trap_Damage[Point]) / 100;
		lpTransSkillAttackData->AttackSize = C_Trap_Range[Point];
		lpTransSkillAttackData->AttackState = 3;
		return TRUE;
	}
...
int	rsRecordCharDamage(rsPLAYINFO* lpPlayInfo, smCHAR* lpChar, int Damage)
{
	...
	DamageList[lpChar->dwObjectSerial][lpPlayInfo] += Damage;
