...
void HoMessage::DrawClanMark(LPDIRECT3DTEXTURE9 clanMark, char* clanName, int x, int y, BOOL frameFlag)
{
	...
	clanMark->GetLevelDesc(0, &desc);

	DrawSprite(x + 4, y + 4, clanMark, 0, 0, desc.Width, desc.Height, true);

	if (clanName != NULL)
		DrawClanName(x, y, clanName);
}

void HoMessage::DrawPatentMessage(int x, int y, char* spatent, char* sname, DWORD color1, DWORD color2, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex)
{
	char MessageOne[64];
	char MessageTwo[64];

	if (spatent != NULL)
		strcpy(MessageOne, spatent);

	if (sname != NULL)
		strcpy(MessageTwo, sname);
	...
	int messageLen = messageTempLen1 + messageTempLen2 + 4;
	...
	DrawCharacterMessageFrame(x, y, messageWidth + (clanMark ? 7 : 0), messageHeight, false, selected);
	if (bellaMarkIndex != -1)
	...
	if (messageTempLen1 > 1)
	{
		if (clanMark == NULL)
			...
			dsTextLineOut(x + 12, y + 3, MessageOne, messageTempLen1);
		else
			dsTextLineOut(x + 9 + 6 * 6, y + 5 + 2, MessageOne, messageTempLen1);
	}
	...
	{
		if (!clanMark)
			//dsTextLineOut(hdc, x + 9 + messageWidth * 6 / 2 - messageTempLen2 * 6 / 2, y + 5 + 2, MessageTwo, messageTempLen2);
			dsTextLineOut(x + 8 + (messageTempLen1 * 7), y + 3, MessageTwo, messageTempLen2);
		else
			dsTextLineOut(x + messageTempLen1 + 9 + 36 + messageWidth * 6 / 2 - messageTempLen2 * 6 / 2, y + 5 + 2, MessageTwo, messageTempLen2);
	}
...
void HoMessage::DrawPatentWithClanMessage(int x, int y, char* sclan, char* spatent, char* sname, DWORD color1, DWORD color2, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex)
{
	char MessageOne[64];
	char MessageTwo[64];
	...
	char* Text = 0;
	if (sclan != NULL)
		strcpy(MessageOne, sclan);

	if (spatent != NULL)
		strcpy(MessageTwo, spatent);

	if (sname != NULL)
		strcpy(MessageThree, sname);
	...
	messageLen = messageTempLen2 + messageTempLen3 + 4 + 5;
	...
	if (messageTempLen1 > 1)
	{
		dsTextLineOut(x + ((clanMark) ? 18 : 10) + messageWidth * 6 / 2 - messageTempLen1 * 6 / 2, y + 5, MessageOne, lstrlen(MessageOne));
	}
	...
	if (messageTempLen2 > 0)
	{
		dsTextLineOut(x + 12 + 32, y + 20, MessageTwo, messageTempLen2);
	}
	...
	{
		dsTextLineOut(x + 8 + 32 + (messageTempLen2 * 7), y + 20, MessageThree, messageTempLen3);
	}

	if (clanMark != NULL)
	{
		if (MessageFrameSelectFlag)
			DrawClanMark(clanMark, NULL, x + 4, y);
		//DrawClanMark(clanMark, NULL, x + (((messageWidth * 6)) / 2) - 4 - messageTempLen1 * 6 / 2, y + 2);
	}
}

void HoMessage::DrawTwoLineMessage(int x, int y, char* message1, char* message2, DWORD color1, DWORD color2, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex, int Type)
{
	char MessageOne[64];
	char MessageTwo[64];
	...
	if (messageTempLen2 > 0)
	{
		if (!clanMark)
			...
			dsTextLineOut(x + 9 + 36 + messageWidth * 6 / 2 - messageTempLen2 * 6 / 2, y + 5 + 2 + 16, MessageTwo, messageTempLen2);
	}
	...
	{
	case 1:
		DrawClanName(x - 3 + (messageWidth / 2 * 6 + 13) - (13 * 5.5 + 32) / 2, y, "Jogador Beta");
		break;
	case 2:
		DrawClanName(x - 3 + (messageWidth / 2 * 6 + 13) - (13 * 5.5 + 32) / 2, y, "GameMaster");
		break;
	}
}
...
void HoMessage::DrawCharacterMessage(int x, int y, char* message, int lineSize, LPDIRECT3DTEXTURE9 clanMark, char* clanName, DWORD color1, BOOL selected, int bellaMarkIndex, int Type)
{
	...
	{
		int size = GetTextFullSize(textdata[i]);
		if (textsize.x < size) textsize.x = size;
		if (textsize.y < 15) textsize.y = 15;
	}
	...
	if (MessageFlag)
	{
		...
		for (int m = 0; m < linesize; m++)
		{
			dsTextLineOut(x + 9, y + 4 + m * textsize.y, textdata[m], lstrlen(textdata[m]));
		}
#else
#ifdef _LANGUAGE_ARGENTINA
		for (int m = 0; m < linesize; m++)
		...
		if (lineSize != 0)
		{
			...
			dsTextLineOut(x + 9, y + 5 + 2, DrawMessageBuffer, lstrlen(DrawMessageBuffer));
	...
	if (CharNameFlag)
	{
...
#else
#ifdef _LANGUAGE_BRAZIL
		dsTextLineOut(x + 9, y + 4, cname, lstrlen(cname));
#else
#ifdef _LANGUAGE_ARGENTINA
		dsTextLineOut(x + 9, y + 5 + 2, cname, lstrlen(cname));
#else
		...
		if (ClanMarkFlag == false)
			dsTextLineOut(x + 9, y + 5 + 2, DrawCharacterNameBuffer, lstrlen(DrawCharacterNameBuffer));

		else
			dsTextLineOut(x + 9 + 6 * 3, y + 5 + 2, DrawCharacterNameBuffer, lstrlen(DrawCharacterNameBuffer));
...
void HoMessage::DrawShopSellMessage(int x, int y, char* message, int lineSize, LPDIRECT3DTEXTURE9 clanMark, DWORD color1, BOOL selected)
{
	...
	for (int i = 0; i < linesize; i++)
	{
		if (textsize.x < GetTextFullSize(textdata[i])) textsize.x = GetTextFullSize(textdata[i]);
		if (textsize.y < 15) textsize.y = 15;
	}
...
int DrawCharacterMessage(int x, int y, char* message, int lineSize, LPDIRECT3DTEXTURE9 clanMark, char* clanName, DWORD color1, BOOL selected, int bellaMarkIndex, int Type)
{
	ChatMessage.DrawCharacterMessage(x, y, message, lineSize, clanMark, clanName, color1, selected, bellaMarkIndex, Type);
	return TRUE;
}

int DrawPatentMessage(int x, int y, char* spatent, char* sname, DWORD color1, DWORD color2, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex)
{
	ChatMessage.DrawPatentMessage(x, y, spatent, sname, color1, color2, clanMark, selected, bellaMarkIndex);
	return TRUE;
}
int DrawPatentWithClanMessage(int x, int y, char* sclan, char* spatent, char* sname, DWORD color1 = RGB(0, 0, 0), DWORD color2 = RGB(0, 0, 0), LPDIRECT3DTEXTURE9 clanMark = NULL, BOOL selected = FALSE, int bellaMarkIndex = -1)
{
	ChatMessage.DrawPatentWithClanMessage(x, y, sclan, spatent, sname, color1, color2, clanMark, selected, bellaMarkIndex);
	return TRUE;
}

int DrawTwoLineMessage(int x, int y, char* messageOne, char* messageTwo, DWORD color1, DWORD color2, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex, int Type)
{
