...
static int JobCharacterPos[6][2] =
{
	{ -90, -320},
	{ -55, -330},
	{ -20, -330},
...
static char* szTempAssassinBodyName[] =
{
	"char\\tmABCD\\e001.ini",
	"char\\tmABCD\\e002.ini",
	"char\\tmABCD\\e003.ini",
...
static char* szTempMartialBodyName[] =
{
	"char\\tmABCD\\f001.ini",
	"char\\tmABCD\\f002.ini",
	"char\\tmABCD\\f003.ini",
...
char* szTempAssassinFaceName[2][3] =
{
	{
...
char* szTempMartialFaceName[2][3] =
{
	{	"char\\tmABCD\\tfh-F01.inf",
		"char\\tmABCD\\tfh-F02.inf",
		"char\\tmABCD\\tfh-F03.inf"
	},
	{
		"char\\tmABCD\\tfh-F01a.inf",
		"char\\tmABCD\\tfh-F02a.inf",
		"char\\tmABCD\\tfh-F03a.inf"
	}
};
...
static int TempFaceCount[6] =
{ 3, 3, 3, 3, 3, 3 };

static int	TempFacePos[6][4] =
{
...
int HoLogin::Load()
{
	...
	TempFaceImage[5][0][0] = CreateTextureMaterial("StartImage\\Login\\TfF01-a.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	TempFaceImage[5][0][1] = CreateTextureMaterial("StartImage\\Login\\TfF01-b.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	TempFaceImage[5][1][0] = CreateTextureMaterial("StartImage\\Login\\TfF02-a.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	TempFaceImage[5][1][1] = CreateTextureMaterial("StartImage\\Login\\TfF02-b.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	TempFaceImage[5][2][0] = CreateTextureMaterial("StartImage\\Login\\TfF03-a.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	TempFaceImage[5][2][1] = CreateTextureMaterial("StartImage\\Login\\TfF03-b.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	...
	TempscronJobImage[5][0] = CreateTextureMaterial("StartImage\\Login\\CharSelect\\T_cls-ma_.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	...
	TempscronJobImage[5][1] = CreateTextureMaterial("StartImage\\Login\\CharSelect\\T_cls-ma.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	...
	TempscronJobTextImage[5] = CreateTextureMaterial("StartImage\\Login\\CharSelect\\T_chr-ma.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
...
{
	"game\\images\\Loading\\1.png",
	"game\\images\\Loading\\2.png",
	"game\\images\\Loading\\3.png",
	"game\\images\\Loading\\4.png",
	"game\\images\\Loading\\5.png",
	"game\\images\\Loading\\6.png",
};

static char* LoadingImageNameBlur[MAX_LOADING_IMAGE] =
{
	"game\\images\\Loading\\1-blur.png",
	"game\\images\\Loading\\2-blur.png",
	"game\\images\\Loading\\3-blur.png",
	"game\\images\\Loading\\4-blur.png",
	"game\\images\\Loading\\5-blur.png",
	"game\\images\\Loading\\6-blur.png",
};

void HoLogin::LoadLoadingImage(int level)
{
	int Rand = rand() % MAX_LOADING_IMAGE;
	GameMapImage = LoadDibSurfaceOffscreen(LoadingImageName[Rand]);
	GameMapImageBorder = LoadDibSurfaceOffscreen("game\\images\\Loading\\border.png");
	GameMapImageBlur = LoadDibSurfaceOffscreen(LoadingImageNameBlur[Rand]);
}
...
int HoLogin::Main()
{
	...
	if (SelectCurrentState == START_GAME_STATE && FadeCurrentState == FADE_IN)
	{
		...
		case SUB_SELECT_CHARACTER_NEW_SERVER:
		{
			...
			{
				memcpy(CharacterName1, szTempAssassinBodyName[0], sizeof(CharacterName1));
				memcpy(CharacterName2, szTempAssassinFaceName[0][JobFaceSelectNum], sizeof(CharacterName2));
			}
			...
			case TRIBE_Tempscron:
				...
				else if (JobBodySelectNum == 5)
				{
					memcpy(CharacterName1, szTempMartialBodyName[0], sizeof(CharacterName1));
					memcpy(CharacterName2, szTempMartialFaceName[0][JobFaceSelectNum], sizeof(CharacterName2));
				}
...
int HoLogin::DrawGameFrame()
{
	...
	if (SelectNextState != START_GAME_STATE)
	{
		...
		for (int index = 0; index < lineWidthCount; index++)
		{
...
{
	DrawSprite(0, 0, GameMapImageBlur, 0, 0, smScreenWidth, smScreenHeight);
	DrawSprite((smScreenWidth >> 1) - (898 >> 1), (smScreenHeight >> 1) - (698 >> 1), GameMapImageBorder, 0, 0, 898, 698);
...
int HoLogin::DrawCharacterInfo()
{
	...
	if (DownUserCharacterInfo[MouseMoveCharacterNum].ClanFlag)
	{
		...
		dsTextLineOut(textX + 17, textY - 3, DownUserCharacterInfo[MouseMoveCharacterNum].ClanName, strlen(DownUserCharacterInfo[MouseMoveCharacterNum].ClanName));
	}
	...
	dsTextLineOut(textX, textY + 37, buffer, strlen(buffer));

	SetFontTextColor(RGB(255, 250, 235));
	dsTextLineOut(textX, textY + 17, DownUserCharacterInfo[MouseMoveCharacterNum].Name, strlen(DownUserCharacterInfo[MouseMoveCharacterNum].Name));

	int jobCode = DownUserCharacterInfo[MouseMoveCharacterNum].Job;
	jobCode = jobCode & 0xFFFF;
	memset(buffer, 0, sizeof(buffer));
	if (jobCode <= 4 || jobCode == 9 || jobCode == 11) {
		wsprintf(buffer, "%s", HoTextTempskron);
	}
	else if (jobCode <= 8 || jobCode == 10) {
		wsprintf(buffer, "%s", HoTextMoryon);
	}
	dsTextLineOut(textX, textY + 57, buffer, strlen(buffer));
	...
	dsTextLineOut(textX, textY + 77, buffer, strlen(buffer));

	if (DownUserCharacterInfo[MouseMoveCharacterNum].ClanFlag)
	{
		if (DownUserCharacterInfo[MouseMoveCharacterNum].ClanMark != NULL)
		{
			DrawSprite(textX - 1, textY - 4, DownUserCharacterInfo[MouseMoveCharacterNum].ClanMark, 0, 0, 16, 16);
		}
	}
...
int HoLogin::KeyFilter()
{
	...
	if (hFocusWnd != NULL && JobFaceSelectNum != -1 && hTextWnd != NULL)
	{
		...
		for (index = 0; index < (int)lstrlen(UserCharacterNewName); index++)
		{
#ifdef	_LANGUAGE_VEITNAM
...
int HoLogin::Draw()
{
			...
			if (USE_MOR == 0)
			{
					...
					&& RaceMarkBlendCount >= 200 && ErrorSelect == ERROR_NONE)
				{
					...
					if (MouseButton[0])
					{
						...
						JobCharacter[JobCharacterCount].smCharInfo.JOB_CODE = 9;
						AddJobCharacter(szTempAssassinBodyName[0], szTempAssassinFaceName[0][0]);

						JobCharacter[JobCharacterCount].smCharInfo.JOB_CODE = 11;
						AddJobCharacter(szTempMartialBodyName[0], szTempMartialFaceName[0][0]);
			...
			else
			{
					...
					&& RaceMarkBlendCount >= 200 && ErrorSelect == ERROR_NONE)
				{
					...
					if (MouseButton[0])
					{
						...
						JobCharacter[JobCharacterCount].smCharInfo.JOB_CODE = 10;
	...
	if (SelectCurrentState == SelectNextState && FadeCurrentState == FadeNextState)
	{
		...
		else if (SelectCurrentState == FACE_SELECT_STATE)
		{
			if (FaceSelectFirstFlag == FALSE && JobFaceSelectNum != -1)
			{
				...
				case TRIBE_Tempscron:
					if (JobBodySelectNum == 0)
						...
						SetChangeJobFace(szTempAssassinBodyName[0], szTempAssassinFaceName[0][JobFaceSelectNum], JobBodySelectNum);
					else if (JobBodySelectNum == 5)
						SetChangeJobFace(szTempMartialBodyName[0], szTempMartialFaceName[0][JobFaceSelectNum], JobBodySelectNum);
			...
			dsDrawTexImage(FaceBoxBigImage[1], newPosX, newPosY + newWidth - 107, newWidth, newHeight, 255);
						...
						if (MouseButton[0])
						{
							...
							if (JobFaceSelectNum != index)
							{
								...
								if (JobBodySelectNum == 0)
									...
									SetChangeJobFace(szTempAssassinBodyName[0], szTempAssassinFaceName[0][index], JobBodySelectNum);
								else if (JobBodySelectNum == 5)
									SetChangeJobFace(szTempMartialBodyName[0], szTempMartialFaceName[0][index], JobBodySelectNum);
