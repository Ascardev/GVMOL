...
void HoEffectType_Attach_Avater::Main(float elapsedTime)
{
	...
	if (m_pAttachChar)
	{
		...
		if (m_iCode != WING_EFFECT1 && m_iCode != WING_EFFECT2 && m_iCode != WING_EFFECT3)
		{
			point3 pos;
			pos.x = (float)m_pAttachChar->pX / 256.f;
			pos.y = (float)m_pAttachChar->pY / 256.f + m_fHeight;
		...
		{
			if (m_pAttachChar->Pattern && m_pAttachChar->Flag && m_pAttachChar->FlagShow && m_pAttachChar->DisplayFlag && m_pAttachChar->MotionInfo)
			{
				if (m_pAttachChar->BackObjBip[0] && m_pAttachChar->BackObjBip[1] && m_pAttachChar->BackObjBip[2] && m_pAttachChar->BackObjBip[0]->pParent)
				{
					partPos.x = (float)m_pAttachChar->Angle.x;
					partPos.y = (float)m_pAttachChar->Angle.y;
					partPos.z = (float)m_pAttachChar->Angle.z;

					point3 Ang;
					Ang.y = (float)partPos.y;
					Ang.x = 0;
					Ang.z = 0;

					if (m_pAttachChar->MotionInfo->State == CHRMOTION_STATE_ATTACK || m_pAttachChar->MotionInfo->State == CHRMOTION_STATE_SKILL)
					{
						partPos.x = (float)(m_pAttachChar->pX + m_pAttachChar->matWing4Skill._41 + 1) / 256.f;
						partPos.y = (float)(m_pAttachChar->pY + m_pAttachChar->matWing4Skill._43 - 4000) / 256.f;
						partPos.z = (float)(m_pAttachChar->pZ + m_pAttachChar->matWing4Skill._42 + 1) / 256.f;
					}
					else
					{
						partPos.x = (float)(m_pAttachChar->pX + m_pAttachChar->matWing._41 + 1) / 256.f;
						partPos.y = (float)(m_pAttachChar->pY + m_pAttachChar->matWing._43 - 3000) / 256.f;
						partPos.z = (float)(m_pAttachChar->pZ + m_pAttachChar->matWing._42 + 1) / 256.f;
					}

					m_pEffectGroup->SetCalcTranslate((point3*)&partPos, &Ang);
					m_fElapsedTime += elapsedTime;
					if (m_iParticleID != -1)
					{
						partPos.x = (float)m_pAttachChar->pX;
		...
		HoEffectManager::GetInstance()->StopEffect(m_iCode, m_pAttachChar);
	}
}
...
void HoEffectType_MusPel::OnEvent(int curState, int curEvent)
{
	if (curState == MUSPEL_SPAWN)
	{
		if (curEvent == EVENT_INIT)
		{
			...
			else if (dwskillCode == SKILL_RAMIEL)
			{
				m_Alpha = 60.f;
				POINT3D pos;
				pos.x = m_pAttachChar->pX;
				...
				m_ParticleID = g_NewParticleMgr.Start("Skill5PriestessRamiel", pos);
				pos.y += 5000;
				g_NewParticleMgr.Start("Skill5PriestessRamielStart", pos);
				m_fCurrentFrame = 10.f * 160.f;
			}
		}
				...
				if (dwskillCode == SKILL_A_MIGAL)
				{
					...
					if (dwskillCode == SKILL_A_MIDRANDA)
					{
						...
						if (dwskillCode == SKILL_RAMIEL)
						{
							...
							pos.y = (int)m_CurrentPos.y;
							pos.z = (int)m_CurrentPos.z;
							g_NewParticleMgr.Start("Skill5PriestessRamielEnd", pos);
						}
		}
	...
	if (curState == MUSPEL_ATTACK)
	{
		...
		else if (curEvent == EVENT_UPDATE)
		{
			if ((m_fCurrentFrame >= 330.f * 160.f) && m_bHandEffectFlag == false)
			{
				...
				pos.z = (int)m_CurrentPos.z;
				if (dwskillCode == SKILL_RAMIEL)
					m_HandParticleNum = g_NewParticleMgr.Start("Skill5PriestessRamielHand", pos);
				else
			...
			if ((m_fCurrentFrame >= 340.f * 160.f) && m_bHitEffectFlag == false && m_AttackMonster != NULL)
			{
				...
				if (dwskillCode == SKILL_RAMIEL)
					g_NewParticleMgr.Start("Skill5PriestessRamielHit1", pos);
				else
...
void HoEffectType_MusPel::Init(smCHAR* pChar, float endTime, float attackTime, int dwSkillCode)
{
	...
	case SKILL_A_MIDRANDA:
		...
		break;
	case SKILL_RAMIEL:
		m_pMesh = (smPAT3D*)AssaSearchRes("Ramiel.ASE", SMMAT_BLEND_ALPHA);
		break;
	}
	...
	case SKILL_A_MIDRANDA:
		...
		break;
	case SKILL_RAMIEL:
		bone = (smPAT3D*)AssaSearchRes("b_Ramiel.ASE", SMMAT_BLEND_ALPHA);
		break;
	}
...
bool HoEffectManager::StopEffect(int code, smCHAR* pAttachChar)
{
	std::vector<HoEffectTypeInterface*>::iterator i;
