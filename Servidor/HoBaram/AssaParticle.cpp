...
void cASSAPARTSHOT::Draw(POINT3D* pPosi, POINT3D* pAngle)
{
	...
	else if (EffectType == ASSA_SHOT_LASER || EffectType == ASSA_SHOT_LASER2 || EffectType == ASSA_SHOT_LASER3 || EffectType == SKILL_JUDGE || EffectType == SKILL_T_CANNON)
	{
...
void cASSAPARTSHOT::Main()
{
	...
	else if (EffectType == ASSA_SHOT_SPARK || EffectType == ASSA_SHOT_SPARK1 || EffectType == ASSA_SHOT_PARKSPARK)
	{
		...
		if (length < 10 && State == 0) //충돌을 했을 경우..
		{
			State = 1;
...
void cASSAPARTSHOT::Start(POINT3D* pCurPosi, POINT3D* pDestPosi, int type)
{
	...
	else if (EffectType == SKILL_FIRE_JAVELIN)
	{
		...
		int step = 12 * fONE;				//창이나 기타 던지는 무기
		GetMoveLocation(0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
		Posi.x += GeoResult_X;
		Posi.y += GeoResult_Y;
		Posi.z += GeoResult_Z;
...
bool HoGetWeaponPos(smCHAR* character, WEAPON_PART_POS* partPos)
{
	...
	{
		for (index = 0; index < partPos->posCount; index++)
		{
...
AssaSkill5PikemanFinalSpear::AssaSkill5PikemanFinalSpear()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkill5PikemanFinalSpear::Start(smCHAR* character, int liveCount, char* szParticleFileName1, char* szParticleFileName2)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start(szParticleFileName1, Posi);
	ParticleIDExt = g_NewParticleMgr.Start(szParticleFileName2, Posi);
	EndFlag = FALSE;
	Max_Time = 70 * (liveCount + 2);

	SearchObj = character->AnimPattern->GetObjectFromName("Bip weapon01");
}

void AssaSkill5PikemanFinalSpear::Main()
{
	if (ParticleID == -1)
		return;
...
AssaSkill5PikemanAmplified::AssaSkill5PikemanAmplified()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void  AssaSkill5PikemanAmplified::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	Posi.y = pChar->pY;
	Posi.z = pChar->pZ;
	ParticleID = g_NewParticleMgr.Start("Skill5PikemanAmplified", Posi);

	Max_Time = liveCount * 70 + 140;
	CODE = SKILL_AMPLIFIED;
	EndFlag = FALSE;
}

void AssaSkill5PikemanAmplified::Main()
{
	if (ParticleID == -1)
		return;
...
AssaSkillTalariaMember::AssaSkillTalariaMember()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void  AssaSkillTalariaMember::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	Posi.y = pChar->pY;
	Posi.z = pChar->pZ;
	ParticleID = g_NewParticleMgr.Start("Skill5_Atalanta_Talaria_KeepEffect", Posi);
...
void AssaSkillTalariaMember::Main()
{
	if (ParticleID == -1)
		return;
...
AssaSkill5AtalantaGalaxyCoup::AssaSkill5AtalantaGalaxyCoup()
{
...
void AssaSkill5AtalantaGalaxyCoup::Start(POINT3D* pCur, POINT3D* pDest, int delay)
{
	...
	Max_Time = 500;
	PatMesh = (smPAT3D*)AssaSearchRes("At5_GalaxyCoup.ASE", SMMAT_BLEND_LAMP);
	POINT3D meshAngle;
...
void AssaSkill5AtalantaGalaxyCoup::Main()
{
	if (Max_Time == Time)
		return;
	if (Time > Delay)
	...
	int step = 10 * fONE;				//창이나 기타 던지는 무기
	GetMoveLocation(0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
	Posi.x += GeoResult_X;
	Posi.y += GeoResult_Y;
	Posi.z += GeoResult_Z;
	...
	{
		Time = Max_Time;
		int num = g_NewParticleMgr.Start("Skill5AtalantaGalaxyCoup", DesPosi);
		g_NewParticleMgr.SetAngle(num, ShootingAngle);
	}
}

void AssaSkill5AtalantaGalaxyCoup::Draw(POINT3D* pPosi, POINT3D* pAngle)
{
	if (PatMesh != NULL)
	{
...
AssaSkill5PiercingIce::AssaSkill5PiercingIce()
{
	DestChar = NULL;
	ParticleID = -1;
	ParticleIDExt1 = -1;
...
void AssaSkill5PiercingIce::Main()
{
	if (Time < 0)
		return;
	if (ParticleID == -1)
	...
	float length = term.x * term.x + term.y * term.y + term.z * term.z;
	length = (float)sqrt(length);
	if (length < 15 || Time > 100) //충) //충돌을 했을 경우..
	{
		if (ParticleID != -1)
		{
		...
		if (ParticleIDExt1 != -1)
		{
			...
			g_NewParticleMgr.Start("Skill5PriestessPiercingIceHit", partPos);
		}
	}
...
void AssaSkill5PiercingIce::Start(POINT3D* pCur, smCHAR* destChar, D3DXVECTOR3* velocity, int delay)
{
	Pos.x = (float)pCur->x;
	Pos.y = (float)pCur->y;
	Pos.z = (float)pCur->z;
	...
	pos.y = (int)Pos.y;
	pos.z = (int)Pos.z;
	ParticleID = g_NewParticleMgr.Start("Skill5PriestessPiercingIce1", pos);
	ParticleIDExt1 = g_NewParticleMgr.Start("Skill5PriestessPiercingIce2", pos);

	Max_Time = 2000;
	Time = -delay;
}

AssaSkill5PristessBlessingKrishna::AssaSkill5PristessBlessingKrishna()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkill5PristessBlessingKrishna::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	Posi.y = pChar->pY;
	Posi.z = pChar->pZ;
	ParticleID = g_NewParticleMgr.Start("S5_Pr_BlessingKrishna", Posi);
...
void AssaSkill5PristessBlessingKrishna::Main()
{
	if (ParticleID == -1)
		return;
...
AssaHyperSonicEffect::AssaHyperSonicEffect()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	LocalAngle = 0;
...
void AssaHyperSonicEffect::Start(smCHAR* character, int LiveTime)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleIDGroup[0] = g_NewParticleMgr.Start("Skill5_Mecanician_HyperSonic1", charPos);
	ParticleIDGroup[1] = g_NewParticleMgr.Start("Skill5_Mecanician_HyperSonic2", charPos);
	ParticleIDGroup[2] = g_NewParticleMgr.Start("Skill5_Mecanician_HyperSonic1", charPos);
	ParticleIDGroup[3] = g_NewParticleMgr.Start("Skill5_Mecanician_HyperSonic2", charPos);

	CODE = SKILL_H_SONIC;
}

void AssaHyperSonicEffect::RespawnParticle()
{
...
void AssaHyperSonicEffect::StopParticle()
{
	//파티클을 다끈다.
	for (int index = 0; index < 4; index++)
	{
...
void AssaHyperSonicEffect::StopParticleID()
{
	for (int index = 0; index < 4; index++)
	{
		if (ParticleIDGroup[index] != -1)
...
void AssaHyperSonicEffect::Main()
{
	if (MyCharacterFlag)
	{
		if (lpCurPlayer != pChar)
	...
	if (pChar->HvRightHand.dwItemCode != 0)
	{
		...
		else if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWS1)
		{
			...
			int cnt;
			if (pChar->HvRightHand.dwItemCode != 0) //활을 든경우 보정..
			{
...
AssaSkill5MecanicianPowerEnhance::AssaSkill5MecanicianPowerEnhance()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkill5MecanicianPowerEnhance::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start("Skill5_Mecanician_PowerEnhance1", Posi);

	ParticleIDExt = g_NewParticleMgr.Start("Skill5_Mecanician_PowerEnhance2", Posi);
...
void AssaSkill5MecanicianPowerEnhance::Main()
{
	if (ParticleID == -1)
		return;
	...
	AnimObjectTree(SearchObj, pChar->frame, 0, (-2048 - (unsigned __int16)-pChar->Angle.y) & 0xFFF, 0);

	D3DXVECTOR3 charPos;
	charPos.x = (float)pChar->pX + (float)SearchObj->mWorld._41;
	charPos.y = (float)pChar->pY + (float)SearchObj->mWorld._43;
	charPos.z = (float)pChar->pZ + (float)SearchObj->mWorld._42;
...
AssaSkill5KnightCrescentMoon::AssaSkill5KnightCrescentMoon()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkill5KnightCrescentMoon::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start("Skill5_Knight_CrescentMoon1", Posi);

	ParticleIDExt = g_NewParticleMgr.Start("Skill5_Knight_CrescentMoon1", Posi);

	Max_Time = 70 * liveCount + 40;
...
void AssaSkill5KnightCrescentMoon::Main()
{
	if (ParticleID == -1)
		return;
...
AssaSkill5AcherEvadeShot::AssaSkill5AcherEvadeShot()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkill5AcherEvadeShot::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start("Skill5_Acher_EvadeShot2", Posi);

	ParticleIDExt = g_NewParticleMgr.Start("Skill5_Acher_EvadeShot3", Posi);
...
void AssaSkill5AcherEvadeShot::Main()
{
	if (ParticleID == -1)
		return;
...
AssaSkill5ThunderRoop::AssaSkill5ThunderRoop()
{
	memset(&ShootingAngle, 0, sizeof(ShootingAngle));
	AniIndex = 0;
	AniDelay = 5;
...
void AssaSkill5ThunderRoop::StartShort(POINT3D* pCur, POINT3D* pDest)
{
	Posi = *pCur;
	Posi.y -= 1000;
	DesPosi = *pDest;
...
void AssaSkill5ThunderRoop::Start(POINT3D* pCur, POINT3D* pDest)
{
	Posi = *pCur;
	Posi.y -= 1000;
	DesPosi = *pDest;
...
void AssaSkill5ThunderRoop::Main()
{
	if (Max_Time == Time)
		return;

	int step = 7 * fONE;				//창이나 기타 던지는 무기
	GetMoveLocation(0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
	Posi.x += GeoResult_X;
	Posi.y += GeoResult_Y;
	Posi.z += GeoResult_Z;
	...
	{
		Time = Max_Time;
		AssaParticle_ThunerRoopShot(&Posi);
		if ((rand() % 1) == 0)
...
void AssaSkill5ThunderRoop::Draw(POINT3D* pPosi, POINT3D* pAngle)
{
	if (!Face.MatNum)return;
...
void AssaCircleTrap::Draw(POINT3D* pPosi, POINT3D* pAngle)
{
	if (PatMesh != NULL)
	{
		smRender.Init();
...
void AssaCircleTrap::Start(POINT3D* pCur, POINT3D* pDest, int shellType)
{
	Posi.x = pCur->x;
	Posi.y = pCur->y;
	Posi.z = pCur->z;
	...
	if (m_ShellType != 3)
		ParticleID = g_NewParticleMgr.Start("Skill5ArcherCircleTrap1", Posi);
...
AssaSkill5KnightHolyVenedicShot::AssaSkill5KnightHolyVenedicShot()
{
	ParticleID = -1;
	MyCharacterFlag = FALSE;
}
...
void AssaSkill5KnightHolyVenedicShot::Start(smCHAR* character)
{
	pChar = character;
	POINT3D charPos;
	charPos.x = pChar->pX;
	...
	ParticleID = g_NewParticleMgr.Start("Skill5_Knight_HolyBenedic2", charPos);
	Max_Time = 5000;
...
void AssaSkill5KnightHolyVenedicShot::Main()
{
	if (ParticleID != -1)
	{
...
AssaSkill5KnightHolyVenedic::AssaSkill5KnightHolyVenedic()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkill5KnightHolyVenedic::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	Posi.y = pChar->pY;
	Posi.z = pChar->pZ;
	ParticleID = g_NewParticleMgr.Start("Skill5_Knight_HolyBenedic", Posi);

	Max_Time = liveCount * 70 + 140;
	CODE = SKILL_H_BENEDIC;
	EndFlag = FALSE;
}

void AssaSkill5KnightHolyVenedic::Main()
{
	if (ParticleID == -1)
		return;
...
AssaSkill5KnightHolyVenedicPartHand::AssaSkill5KnightHolyVenedicPartHand()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkill5KnightHolyVenedicPartHand::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start("Skill5KnightHolyBenedicHand1", Posi);
	ParticleIDExt = g_NewParticleMgr.Start("Skill5KnightHolyBenedicHand2", Posi);
	EndFlag = FALSE;
...
void AssaSkill5KnightHolyVenedicPartHand::Main()
{
	if (ParticleID == -1)
		return;
...
void cASSAChar::Main()
{
	...
	if (EffectType == HOLY_VALOR_JANG || EffectType == HOLY_VALOR_MEMBER || EffectType == HIGH_LEVEL ||
	...
	if (EffectType == HIGH_LEVEL)
	{
		D3DXVECTOR3 partPos;
		partPos.x = (float)pChar->pX;
		partPos.y = (float)pChar->pY - (float)400;
		partPos.z = (float)pChar->pZ;
	...
	if ((Time + 70 * 3 >= Max_Time))
	{
		if (EffectType == HOLY_VALOR_JANG || EffectType == HOLY_VALOR_MEMBER ||
			EffectType == DRASTIC_SPIRIT_JANG || EffectType == DRASTIC_SPIRIT_MEMBER || EffectType == HIGH_LEVEL)
		{
	...
	if (!MyCharacterFlag)
		if (pChar->Flag)
			if (!pChar->MotionInfo || pChar->MotionInfo->State == CHRMOTION_STATE_DEAD && EffectType != HIGH_LEVEL)
				StopEffect();
	...
	if (!MyCharacterFlag)
		if (pChar->Flag)
			if (EffectType == HIGH_LEVEL /*&& !pChar->TopLvL*/)
				StopEffect();
...
void cASSAChar::Start(smCHAR* character, int code, int maxTime, float delay)
{
	...
	if (EffectType == HIGH_LEVEL /*&& !character->TopLvL*/)
	{
		return;
	}
	...
	else if (EffectType == HIGH_LEVEL)
	{
		Max_Time += 70 * 3;
		PartNum = g_NewParticleMgr.Start("HighLevelUserMerit_Star", Posi);
	}
...
void AssaSkillWeaponMotionBlur::Start2(smCHAR* character, int liveTime, char* resName)
{
	int cnt = 0;
	...
	int matNum = *(int*)AssaSearchRes(resName, SMMAT_BLEND_LAMP);
...
void AssaSkillTriumphOfValhallaPart::StartHallOfValhalla(smCHAR* character, int liveCount)
{
	...
	if (pChar->AnimPattern != NULL) // NULL 값일때 이 밑에 함수를 호출시켜서 GM모드에서 field 명령어로 이동시 뻑남 // 해외
	{
		ObjBip = pChar->AnimPattern->GetObjectFromName("Bip01 R Hand");
...
void AssaSkillTriumphOfValhallaShot::Main()
{
	...
	if (length < 15 || (!DestChar->MotionInfo || DestChar->MotionInfo->State == CHRMOTION_STATE_DEAD) ||
		Time > 100) //충돌을 했거나 시간이 너무 지났을 경우.
	{
		if (ParticleID != -1)
		{
			g_NewParticleMgr.SetRendering(ParticleID, FALSE);
...
void AssaSkillTriumphOfValhallaShot::Start(smCHAR* curChar, smCHAR* destChar, D3DXVECTOR3* velocity, int liveCount)
{
	...
	SearchObj = destChar->AnimPattern->GetObjectFromName("Bip01 R Hand"); //오브젝트를 읽는다
...
void AssaSkill3StormJavelin::Start(POINT3D* pCur, POINT3D* pDest)
{
	...
	int step = 30 * fONE;				//창이나 기타 던지는 무기
	GetMoveLocation(0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
	Posi.x += GeoResult_X;
	Posi.y += GeoResult_Y;
	Posi.z += GeoResult_Z;
...
void AssaSkyParticle::Main()
{
	...
	if (ParticleSpawnFlag == false)
	{
		...
		if (Delay <= 0)
		{
			...
			partPos.y = (int)Pos.y;
			partPos.z = (int)Pos.z;
			if (m_dwSkillCode == 0x50)
				ParticleID = g_NewParticleMgr.Start("Snow", partPos);
			if (m_dwSkillCode == SKILL_P_DEITY)
				ParticleID = g_NewParticleMgr.Start("PressOfDeity2", partPos);
			else
...
void AssaSkyParticle::Start(POINT3D* pCur, POINT3D* pDest, int delay, DWORD dwSkillCode)
{
	Pos.x = (float)pCur->x;
	Pos.y = (float)pCur->y;
	Pos.z = (float)pCur->z;
