...
int SetServerConfig(char* decode, char* lpPoint)
{
	...
	{
		char szBuf[32] = { 0 };
		LeIniStr("Event", "MixFree", "server.ini", szBuf);
		if (lstrcmpi(szBuf, "True") == 0)
		{
			MixEvento = TRUE;
		}
	...
	if (!flag && lstrcmpi(decode, "*LIMIT_ATTACK") == 0) {
		p = GetWord(strBuff, p);
		if (strBuff[0])
			rsServerConfig.LimitAttack = atoi(strBuff);

		flag++;
	}

	if (!flag && lstrcmpi(decode, "*LIMIT_DEFENSE") == 0) {
		p = GetWord(strBuff, p);
		if (strBuff[0])
			rsServerConfig.LimitDefense = atoi(strBuff);

		flag++;
	}

	if (!flag && lstrcmpi(decode, "*LIMIT_ABSORB") == 0) {
		p = GetWord(strBuff, p);
		if (strBuff[0])
			rsServerConfig.LimitAbsorb = atoi(strBuff);

		flag++;
	}

	if (!flag && lstrcmpi(decode, "*LIMIT_DAMAGE") == 0) {
		p = GetWord(strBuff, p);
		if (strBuff[0])
			rsServerConfig.LimitDamage = atoi(strBuff);
...
smCONFIG* smConfigDecode(char* iniFile)
{
	...
	while (!feof(fp))
	{
		...
		while (1) {
			...
			if (SetOdbcConfig(decode, p)) break;
