...
void HellGates::SendAward(rsPLAYINFO* Player)
{
	if (SQL::GetInstance()->IsConnected())
	{
		...
		wsprintf(szQuery, "SELECT * FROM [PristonDB].[dbo].[Roulette_Point] WHERE [Character]='%s'", Player->szName);
		if (SQL::GetInstance()->Execute(szQuery))
		{
			...
			{
				wsprintf(szQuery, "INSERT INTO [PristonDB].[dbo].[Roulette_Point] ([Character],[Point]) values('%s',1)", Player->szName);
			}
...
void HellGates::Main()
{
	...
	if (Flag == 0)
	{
		if ((st.wHour == 03 && st.wMinute == 55) || (st.wHour == 9 && st.wMinute == 55) || (st.wHour == 15 && st.wMinute == 55) || (st.wHour == 21 && st.wMinute == 55))
		{
			Flag = 1;
			OpenBattle();
	...
	else if (Flag == 1)
	{
		if (OverOpenTime < dwPlayServTime)
		{
			Flag = 2;
			StartBattle();
			rsSendServerChatMessageToAllUser(3, "Os Portões do Inferno foram fechados!");
	...
	else if (Flag == 2)
	{
		if (OverBattleTime < dwPlayServTime)
		{
			...
			Flag = 0;
			Round = 0;
			rsSendServerChatMessageToAllUser(3, "> O Evento nos Portões do Inferno acabou de terminar!");
			...
			for (int cnt = 0; cnt < CONNECTMAX; cnt++)
			{
				if (rsPlayInfo[cnt].lpsmSock)
				{
					...
					if (rsPlayInfo[cnt].OnHellGates)
					{
						SendAward(&rsPlayInfo[cnt]);
		...
		if (!Boss)
		{
			if (Round < 5)
			{
				if (SQL::GetInstance()->IsConnected())
				{
					SQL::GetInstance()->EnterSqlSection();
					char szQuery[256];
					wsprintf(szQuery, "SELECT Name FROM [PristonDB].[dbo].[HellGatesBosses] WHERE ID=%d", Round);
					if (SQL::GetInstance()->Execute(szQuery))
					{
						if (SQL::GetInstance()->Fetch())
						{
							...
							Boss = OpenMonsterFromName(szName, -3626 * fONE, 486 * fONE, -43146 * fONE);
						}
					}
					SQL::GetInstance()->LeaveSqlSection();
				}
