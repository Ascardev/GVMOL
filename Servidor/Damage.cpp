...
int dm_SendTransDamage(smCHAR* lpChar, int PowParam1, int PowParam2, int AttackState, int Resistance, DWORD dwSkillCode, int UseAccuracy, smWINSOCK* lpsmSock)
{
	...
	if ((abs((double)dwPlayTime - dwLastMouseMoveTime) > 1024 * 60 * 3 ||
		abs((double)dwPlayTime - dwLastCharMoveTime) > 1024 * 60 * 3) ||
		RestartPlayCount != 0) {
		return FALSE;
	}
	...
	if (UseAccuracy) {
		...
		case smCHAR_STATE_ENEMY:
			...
			if (smConfig.DebugMode && VRKeyBuff[VK_SHIFT]) {
				char szBuff[256];
				wsprintf(szBuff, "È®·ü : %d/%d", rnd, accuracy);
	...
	if (((lpCurPlayer->dwActionItemCode ^ sinWM1) & sinITEM_MASK2) != 0 || ((lpCurPlayer->dwActionItemCode ^ sinWN1) & sinITEM_MASK2) != 0)
...
{
	smWINSOCK* lpsmSendSock = nullptr;
	int result;
