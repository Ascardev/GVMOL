...
void cHASIEGE::Draw()
{
	...
	if ((!haSiegeBoardFlag && lpCurPlayer->OnStageField >= 0 && StageField[lpCurPlayer->OnStageField]->State == FIELD_STATE_CASTLE) || (!haSiegeBoardFlag && lpCurPlayer->OnStageField >= 0 && StageField[lpCurPlayer->OnStageField]->FieldCode != 3 && StageField[lpCurPlayer->OnStageField]->FieldCode != 43 && StageField[lpCurPlayer->OnStageField]->FieldCode != 23 && StageField[lpCurPlayer->OnStageField]->FieldCode != 32 && lpCurPlayer->PvP_Param))
	{
		char szBuff[255];
		int WinX = (WinSizeX / 2);
		DrawFontImage("Battle Point", WinX, 12, D3DCOLOR_RGBA(0, 255, 0, 255), 0.7f);
		DrawFontImage("Death Point", WinX, 31, D3DCOLOR_RGBA(0, 255, 0, 255), 0.7f);

		wsprintf(szBuff, "%d", lpCurPlayer->BattlePoint);
		DrawFontImage(szBuff, WinX - 60, 12, D3DCOLOR_RGBA(200, 0, 0, 255), 0.7f);
		wsprintf(szBuff, "%d", lpCurPlayer->DeathPoint);
		DrawFontImage(szBuff, WinX - 60, 31, D3DCOLOR_RGBA(200, 0, 0, 255), 0.7f);

		if (lpCurPlayer->pZ > -24051 * fONE && lpCurPlayer->pZ < -23708 * fONE && lpCurPlayer->pX < 37678 * fONE && lpCurPlayer->pX > 32915 * fONE)
		{
			DrawFontImage("Warning Zone", WinX - 50, 75, D3DCOLOR_RGBA(255, 255, 0, 255), 0.85f);
		}
		else if (lpCurPlayer->pZ > -23708 * fONE && lpCurPlayer->pX < 37678 * fONE && lpCurPlayer->pX > 32915 * fONE)
		{
			DrawFontImage("Safe Zone", WinX - 35, 75, D3DCOLOR_RGBA(0, 255, 0, 255), 0.85f);
		}
		else if (lpCurPlayer->pZ < -24051 * fONE && lpCurPlayer->pX < 37678 * fONE && lpCurPlayer->pX > 32915 * fONE)
		{
			DrawFontImage("Battle Zone", WinX - 40, 75, D3DCOLOR_RGBA(200, 0, 0, 255), 0.85f);
		}
	}
...
int cHASIEGE::ShowPickUserDeadPoint(int x, int y, int DeadCount)
{
	char TempBuff[32];
	memset(&TempBuff, 0, sizeof(TempBuff));

	DrawFontImage("D.P", x, y, D3DCOLOR_RGBA(0, 255, 0, 255), 0.7f);
	wsprintf(TempBuff, "%d", DeadCount);
	DrawFontImage(TempBuff, x + 24, y, D3DCOLOR_RGBA(255, 0, 0, 255), 0.7f);
