...
int	sinRemodelDelayCount = 0;
int	sinChangeItemDelayCount = 0;
...
int SetInterEffectAging2()
{
	...
	if (cRemodel.OpenFlag)
	{
		for (j = sinRemodelDelayCount; j < sinRemodelDelayCount + 1; j++)
		{
			if (sRemodelItem.RemodelItem[j].Flag)
			{
				sinRemodelDelayCount++;
				...
				for (i = 0; i < SIN_MAX_INTER_EFFECT; i++)
				{
					if (!sInter_Effect[i].Time)
					{
						sInter_Effect[i].Kind = SIN_INTER_EFFECT_AGING2;
						sInter_Effect[i].Time = 1;
						sInter_Effect[i].Face.x = sRemodelItem.RemodelItem[1].x + 5;
						sInter_Effect[i].Face.y = sRemodelItem.RemodelItem[1].y + 5;
						sInter_Effect[i].Face.width = rand() % 5 + 5;
						sInter_Effect[i].Face.height = sInter_Effect[i].Face.width;
						sInter_Effect[i].Max_Time = rand() % 10 + 5;
						sInter_Effect[i].Face.Transparency = 255;
						...
						if (CountTemp >= 24)
						{
							CountTemp = 0;
							sinPlaySound(SIN_SOUND_FAILD_MIXITEM);
							sRemodelItem.RemodelItem[1].x = 10000;
							sRemodelItem.RemodelItem[1].y = 10000;
							cRemodel.ReformRemodelItem();
							break;
						}
					}
				}
