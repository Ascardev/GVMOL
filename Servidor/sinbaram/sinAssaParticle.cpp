...
#define ASSA_PARTICLE_RUSH_BOW			0x50000009
...
int AssaParticle_RushElbow(cASSAEFFECT* pEffect)
{
	POINT3D TempPosi;

	AssaSearchWeaponPos(pEffect->pChar, &pEffect->Posi, 1);
	cAssaEffect[Assa]->Face.MatNum = g_NewParticleMgr.Start("Skill2_MartialArtist_RushElbow_Hit", pEffect->Posi);
	...
	pEffect->CODE = ASSA_PARTICLE_RUSH_BOW;
...
int AssaParticle_SoulBreaker(cASSAEFFECT* pEffect)
{
	POINT3D TempPosi;
	...
	pEffect->MOVECODE = ASSA_MOVE;
	pEffect->MoveSpeed.x = (pEffect->Posi.x - TempPosi.x) / 40;
	pEffect->MoveSpeed.z = (pEffect->Posi.z - TempPosi.z) / 40;
	pEffect->CODE = ASSA_PARTICLE_PIERCING;
...
int sinAssaRushElbow(smCHAR* pChar)
{
	int Temp = 1;
	...
	for (int j = 0; j < 14; j++)
	{
		...
		cAssaEffect[Assa]->Size.w = 500;
		cAssaEffect[Assa]->MoveSpeed.y = -1000;
		AssaGetCirclePosi(cAssaEffect[Assa], 3584, GetRandomPos(0, 4096));
		cAssaEffect[Assa]->MOVECODE = ASSA_MOVE | ASSA_ROTATE | ASSA_CHAR_POSI;
		if (Temp > 0)Temp = -1;
		else Temp = 1;
		cAssaEffect[Assa]->RAngleSpeed = 25 * Temp;
		cAssaEffect[Assa]->Time = j * Temp;
		cAssaEffect[Assa]->FACE_TYPE = ASSAFACE_SCALE;
		cAssaEffect[Assa]->ScalePosi.y = 20000;
	}
