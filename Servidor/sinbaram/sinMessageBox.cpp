...
{
	CheckBox = CreateTextureMaterial("game\\images\\settings\\checkbox.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	CheckBox_Checked = CreateTextureMaterial("game\\images\\settings\\checkbox_.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	...
	MatMyShopPRBoxLine = CreateTextureMaterial("Image\\SinImage\\Inven\\PR_Box.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	...
	for (i = 0; i < 20; i++)
	{
		if (sMessageBox2[i].Flag)
		{
			...
			if (i == MESSAGE_MYSHOP_ITEM) {
				dsDrawTexImage(CheckBox, sMessageBox2[i].x + 33, sMessageBox2[i].y + 36, 15, 15, 255);

				if (KindCurrency == 1) {
					dsDrawTexImage(CheckBox_Checked, sMessageBox2[i].x + 33, sMessageBox2[i].y + 36, 15, 15, 255);
				}

				SetFontTextColor(RGB(255, 255, 255));
				dsTextLineOut(sMessageBox2[i].x + 52, sMessageBox2[i].y + 37, "Coin", 4);

				dsDrawTexImage(CheckBox, sMessageBox2[i].x + 94, sMessageBox2[i].y + 36, 15, 15, 255);

				if (KindCurrency == 2) {
					dsDrawTexImage(CheckBox_Checked, sMessageBox2[i].x + 94, sMessageBox2[i].y + 36, 15, 15, 255);
				}

				dsTextLineOut(sMessageBox2[i].x + 113, sMessageBox2[i].y + 37, "Gold", 4);
			}
...
void cMESSAGEBOX::Main()
{
	...
	if (sLost_Item.Flag) {
		...
		BackStartPos.y = 114;
		sLost_Item.BuyOk = 0;
		if (pRealCursorPos.x > BackStartPos.x + BUTTON2_X + 35 && pRealCursorPos.x < BackStartPos.x + BUTTON2_X + 35 + 32 &&
			pRealCursorPos.y >BackStartPos.y + BUTTON2_BOX_Y && pRealCursorPos.y < BackStartPos.y + BUTTON2_BOX_Y + 16)
			sLost_Item.BuyOk = 1;
	}
	...
	if (sStarShop.Flag)
	{
		...
		{
			if (pRealCursorPos.x > BackStartPos.x + BUTTON2_X + 35 && pRealCursorPos.x < BackStartPos.x + BUTTON2_X + 35 + 32 &&
				pRealCursorPos.y >BackStartPos.y + BUTTON2_Y && pRealCursorPos.y < BackStartPos.y + BUTTON2_Y + 16)
				sStarShop.ButtonClose = 1;

			sStarShop.BuyOk = 0;
			if (pRealCursorPos.x > BackStartPos.x + BUTTON2_X + 35 && pRealCursorPos.x < BackStartPos.x + BUTTON2_X + 35 + 32 &&
				pRealCursorPos.y >BackStartPos.y + 160 && pRealCursorPos.y < BackStartPos.y + 160 + 16)
				sStarShop.BuyOk = 1;
		}
	}
	...
	if (sMessageBox_RecvItem.Flag)
	{
		...
		if (pRealCursorPos.x > BackStartPos.x + BUTTON2_X && pRealCursorPos.x < BackStartPos.x + BUTTON2_X + 32 &&
			pRealCursorPos.y >BackStartPos.y + BUTTON2_Y && pRealCursorPos.y < BackStartPos.y + BUTTON2_Y + 16)
			sMessageBox_RecvItem.ButtonYes = 1;

		if (pRealCursorPos.x > BackStartPos.x + BUTTON3_X && pRealCursorPos.x < BackStartPos.x + BUTTON3_X + 32 &&
			pRealCursorPos.y >BackStartPos.y + BUTTON3_Y && pRealCursorPos.y < BackStartPos.y + BUTTON3_Y + 16)
	...
	for (i = 0; i < 20; i++)
	{
		if (sMessageBox2[i].Flag)
		{
			if (pRealCursorPos.x > sMessageBox2[i].x + 124 && pRealCursorPos.x < sMessageBox2[i].x + 124 + 19 &&
				pRealCursorPos.y >sMessageBox2[i].y + 62 && pRealCursorPos.y < sMessageBox2[i].y + 62 + 13)
				MessageArrowPosi = 1;

			if (pRealCursorPos.x > sMessageBox2[i].x + 124 && pRealCursorPos.x < sMessageBox2[i].x + 124 + 19 &&
				pRealCursorPos.y >sMessageBox2[i].y + 75 && pRealCursorPos.y < sMessageBox2[i].y + 75 + 13)
				MessageArrowPosi = 2;

			if (pRealCursorPos.x > sMessageBox2[i].x + 40 && pRealCursorPos.x < sMessageBox2[i].x + 40 + 32 &&
				pRealCursorPos.y >sMessageBox2[i].y + 95 && pRealCursorPos.y < sMessageBox2[i].y + 98 + 16)
				MessageButtonPosi = 1;

			if (pRealCursorPos.x > sMessageBox2[i].x + 94 && pRealCursorPos.x < sMessageBox2[i].x + 94 + 32 &&
				pRealCursorPos.y >sMessageBox2[i].y + 95 && pRealCursorPos.y < sMessageBox2[i].y + 95 + 16)
				MessageButtonPosi = 2;
		}
	}
	for (i = 0; i < MESSAGE3_MAX; i++)
	{
		if (sMessageBox3[i].Flag)
		{
			if (pRealCursorPos.x > sMessageBox3[i].x + 40 && pRealCursorPos.x < sMessageBox3[i].x + 40 + 32 &&
				pRealCursorPos.y >sMessageBox3[i].y + 95 && pRealCursorPos.y < sMessageBox3[i].y + 98 + 16)
				MessageButtonPosi = 1;

			if (pRealCursorPos.x > sMessageBox3[i].x + 94 && pRealCursorPos.x < sMessageBox3[i].x + 94 + 32 &&
				pRealCursorPos.y >sMessageBox3[i].y + 95 && pRealCursorPos.y < sMessageBox3[i].y + 95 + 16)
				MessageButtonPosi = 2;
		}
	}
...
void cMESSAGEBOX::LButtonDown(int x, int y)
{
	...
	for (i = 0; i < MESSAGE3_MAX; i++)
	{
		if (sMessageBox3[i].Flag)
		{
			...
			if (MessageButtonPosi == 2)
			{
				...
				{
					if (sinChar->JOB_CODE <= 4 || sinChar->JOB_CODE <= 9 || sinChar->JOB_CODE <= 11) {
	...
	for (i = 0; i < 20; i++) {
		if (sMessageBox2[i].Flag) {
			if (i == MESSAGE_MYSHOP_ITEM) {
				if (MouseAction(sMessageBox2[i].x + 33, sMessageBox2[i].y + 36, 15, 15)) {
					KindCurrency = 1;
				}

				if (MouseAction(sMessageBox2[i].x + 94, sMessageBox2[i].y + 36, 15, 15)) {
					KindCurrency = 2;
				}
			}
			...
			if (MessageArrowPosi == 1) {
				...
				{
					if (CoinShop::GetInstance()->GetCoin() < sMessageBox2[i].Count)
					{
						sMessageBox2[i].Count = CoinShop::GetInstance()->GetCoin();
					}
				}
				...
				if (i == MESSAGE_MOVE_MONEY_MOVE) {
					...
					if ((sWareHouse.Money - 2023) + sMessageBox2[i].Count > 1000000000) {
						sMessageBox2[i].Count -= ((sWareHouse.Money - 2023) + sMessageBox2[i].Count) - 1000000000;
					}
				}
				...
				if (i == MESSAGE_MOVE_MONEY_RECV) {
					...
					if (sinChar->Level <= 10) {
						...
						case 3:
							if ((sinChar->Money + sMessageBox2[i].Count) > 1000000000) {
								sMessageBox2[i].Count = 1000000000 - sinChar->Money;
								if (sMessageBox2[i].Count < 0) sMessageBox2[i].Count = 0;
							}
...
int cMESSAGEBOX::ShowMessageEvent(char* Msg)
{
	...
	sMessageBox[49].x = (WinSizeX >> 1) - (64 + sMessageBox[49].Line * 16) / 2;
...
int cMESSAGEBOX::ShowMessage(int Kind)
{
	...
	if (Kind == MESSAGE_MOVE_TRADE_ITEM || Kind == MESSAGE_NOTEXIT_ITEMIMAGE || Kind == MESSAGE_MOVE_COIN_TRADE)
	{
	...
	sMessageBox[Kind].x = (WinSizeX >> 1) - (64 + sMessageBox[Kind].Line * 16) / 2;
...
int cMESSAGEBOX::ShowMessage(const char* message)
{
	...
	sMessageBox[49].x = WinSizeX / 2 - ((sMessageBox[49].len * 10) / 2) + 40;
	sMessageBox[49].y = 5;
...
int cMESSAGEBOX::ShowMessage2(int Kind)
{
	...
	sMessageBox2[Kind].x = (WinSizeX >> 1) - 128;
	sMessageBox2[Kind].y = (WinSizeY >> 1) - 64;
	...
	if (Kind == MESSAGE_MOVE_COIN_TRADE)
	{
		if (sTrade.Coin)
		{
			...
			CheckCharForm();
			TempShowCoin = 0;
			CoinShop::GetInstance()->PlusCoin(sTrade.Coin);
			sTrade.Coin = 0;
			ReformCharForm();
			ReformTrade((void*)&sTrade);
			SendTradeItem(cTrade.TradeCharCode);
...
int cMESSAGEBOX::ShowMessage3(int Kind, char* Doc)
{
	...
	sMessageBox3[Kind].x = (WinSizeX >> 1) - 128;
	sMessageBox3[Kind].y = (WinSizeY >> 1) - 64;
	sinMessageBoxShowFlag = 1;
...
void cMESSAGEBOX::Resize()
{
	for (int i = 0; i < 49; i++)
	{
		if (sMessageBox[i].Flag) sMessageBox[i].x = (WinSizeX >> 1) - (64 + sMessageBox[i].Line * 16) / 2;
	}
	...
	for (int i = 0; i < 20; i++)
	{
		if (sMessageBox2[i].Flag)
		{
			sMessageBox2[i].x = (WinSizeX >> 1) - 128;
			sMessageBox2[i].y = (WinSizeY >> 1) - 64;
		}
	}

	for (int i = 0; i < MESSAGE3_MAX; i++)
	{
		if (sMessageBox3[i].Flag)
		{
			sMessageBox3[i].x = (WinSizeX >> 1) - 128;
			sMessageBox3[i].y = (WinSizeY >> 1) - 64;
		}
	}
}
	...
	if (sinMessageBoxShowFlag || TempFlag) {
		...
		for (i = 0; i < MESSAGE3_MAX; i++) {
			if (sMessageBox3[i].Flag) {
				...
				if (i == MESSAGE_PREMIUM_CANCEL)
				{
					SetFontTextColor(RGB(255, 255, 255));
					TempLenght = (int)((172 - (lstrlen(sMessageBox3[49].MessageDoc1) * 5.5)) / 2 - 5);
					dsTextLineOut(sMessageBox3[i].x + TempLenght, sMessageBox3[i].y + 35 + 8, sMessageBox3[49].MessageDoc1, lstrlen(sMessageBox3[49].MessageDoc1));
				}
				else
				{
					SetFontTextColor(RGB(255, 255, 255));
				...
				else if (i == MESSAGE_PREMIUM_CANCEL)
				{
					TempLenght = (int)((172 - (lstrlen(sMessageBox3[49].MessageDoc2) * 5.5)) / 2 - 5);
					dsTextLineOut(sMessageBox3[i].x + TempLenght + 2, sMessageBox3[i].y + 55 + 5, sMessageBox3[49].MessageDoc2, lstrlen(sMessageBox3[49].MessageDoc2));
				}
				else {
...
int cMESSAGEBOX::CheckOkMessageBox2(int Kind)
{
	...
	case MESSAGE_MASTER_SKILL:
		if (pMasterSkill)
		{
			...
			if (0 <= (sinChar->Money - (sinMasterSkill_Money[pMasterSkill->Skill_Info.SkillNum] + (sinMasterSkill_PlusMoney[pMasterSkill->Skill_Info.SkillNum] * (pMasterSkill->Point))))) {
				...
				LearnSkillEffect(0);
				pMasterSkill->UseSkillCount = 10000;
				SaveGameData();
			}
			else
				ShowMessage(MESSAGE_NOT_ENOUGH_MONEY);
	...
	case MESSAGE_CANCEL_QUEST:
		Quest::GetInstance()->SendCancelToServer();
		break;

	case MESSAGE_MASTER_SKILL_DOWN:
		if (pMasterSkill)
		{
			if (cShop.haBuyMoneyCheck(sinMasterSkill_Money[pMasterSkill->Skill_Info.SkillNum] + (sinMasterSkill_PlusMoney[pMasterSkill->Skill_Info.SkillNum] * (pMasterSkill->Point))))
			{
	...
	case MESSAGE_BUY_COIN_SHOP:
	{
		...
		if (lpsmSock)
		{
			lpsmSock->Send((char*)&cCPacket, cCPacket.size, TRUE);
		}
	}
	break;
	case MESSAGE_BUY_TIME_SHOP:
	{
		smWINSOCK* lpsmSock;
		lpsmSock = GetAreaServerSock();
		...
		if (lpsmSock)
		{
			lpsmSock->Send((char*)&cTPacket, cTPacket.size, TRUE);
		}
	}
	break;
	case MESSAGE_PREMIUM_CANCEL:
	{
		smTRANS_COMMAND	TransCommand;
		TransCommand.code = smTRANSCODE_PREMIUM_CANCEL;
		TransCommand.size = sizeof(smTRANS_COMMAND);
		TransCommand.LParam = chaPremiumitem.SelectItemIndex;

		smWINSOCK* lpsmSock;
		lpsmSock = GetAreaServerSock();

		if (TransCommand.LParam == nsPremiumItem::HELP_PET_TERRY || TransCommand.LParam == nsPremiumItem::HELP_PET_NEPSYS ||
			TransCommand.LParam == nsPremiumItem::HELP_PET_IO || TransCommand.LParam == nsPremiumItem::HELP_PET_IO)
		{
			cHelpPet.ClosePet();
		}

		if (TransCommand.LParam == nsPremiumItem::PHENIX_PET)
		{
			cPCBANGPet.ClosePet();
		}

		if (TransCommand.LParam == nsPremiumItem::VIP)
		{
			PVIP = 0;
			chaPremiumitem.SetVipTime(0);
		}
	...
	case MESSAGE_FORCE_ORB_CANCEL:
	{
		cSkill.CancelContinueSkill(SKILL_FORCE_ORB);
		cInvenTory.SetItemToChar();

		AssaParticle_ShelltomWeaponStop(lpCurPlayer);

		smTRANS_COMMAND	TransCommand;
		TransCommand.code = smTRANSCODE_FORCE_ORB_CANCEL;
		TransCommand.size = sizeof(smTRANS_COMMAND);
	...
	case MESSAGE_QUEST_150A:
	{
		smTRANS_COMMAND	TransCommand;
		TransCommand.code = smTRANSCODE_PROGRESS_QUEST;
		TransCommand.size = sizeof(smTRANS_COMMAND);
		TransCommand.WParam = QUESTID_150A;
		...
		Quest::GetInstance()->Updater(QUESTID_150A);
	}
	break;

	case MESSAGE_QUEST_155A:
	{
		smTRANS_COMMAND	TransCommand;
		TransCommand.code = smTRANSCODE_PROGRESS_QUEST;
		TransCommand.size = sizeof(smTRANS_COMMAND);
		TransCommand.WParam = QUESTID_155A;
		...
		Quest::GetInstance()->Updater(QUESTID_155A);
	}
	break;

	case MESSAGE_QUEST_150B:
	{
		smTRANS_COMMAND	TransCommand;
		TransCommand.code = smTRANSCODE_PROGRESS_QUEST;
		TransCommand.size = sizeof(smTRANS_COMMAND);
		TransCommand.WParam = QUESTID_150B;
		...
		Quest::GetInstance()->Updater(QUESTID_150B);
	}
	break;
	...
	case MESSAGE_QUEST_START:
	{
		...
		TransCommand.code = smTRANSCODE_PROGRESS_QUEST;
		TransCommand.size = sizeof(smTRANS_COMMAND);
		TransCommand.WParam = QUESTID_START;
		...
		Quest::GetInstance()->Updater(QUESTID_START);
	}
	break;

	case MESSAGE_QUEST_TRAJE:
	{
		smTRANS_COMMAND	TransCommand;
		TransCommand.code = smTRANSCODE_PROGRESS_QUEST;
		TransCommand.size = sizeof(smTRANS_COMMAND);
		TransCommand.WParam = QUESTID_TRAJE;
		...
		Quest::GetInstance()->Updater(QUESTID_TRAJE);
	}
	break;

	case MESSAGE_QUEST_BOTA:
	{
		smTRANS_COMMAND	TransCommand;
		TransCommand.code = smTRANSCODE_PROGRESS_QUEST;
		TransCommand.size = sizeof(smTRANS_COMMAND);
		TransCommand.WParam = QUESTID_BOTA;
		...
		Quest::GetInstance()->Updater(QUESTID_BOTA);
	}
	break;

	case MESSAGE_QUEST_AMULETO:
	{
		smTRANS_COMMAND	TransCommand;
		TransCommand.code = smTRANSCODE_PROGRESS_QUEST;
		TransCommand.size = sizeof(smTRANS_COMMAND);
		TransCommand.WParam = QUESTID_AMULETO;
		...
		Quest::GetInstance()->Updater(QUESTID_AMULETO);
	}
	break;

	case MESSAGE_ERAGON_CAVE:
	{
		WarpField2(48);
	}
	break;
...
int cMESSAGEBOX::CheckOkMessageBox(int Kind)
{
	...
	case  MESSAGE_MOVE_COIN_TRADE:
		...
		if (CoinShop::GetInstance()->GetCoin() - sMessageBox2[Kind].Count < 0)
		{
			ExitProcess(0);
			SendSetHackUser2(4100, 2);
			sMessageBox2[Kind].Count = CoinShop::GetInstance()->GetCoin();
		}

		CoinShop::GetInstance()->MinusCoin(sMessageBox2[Kind].Count);
		ReformCharForm();
	...
	case MESSAGE_MYSHOP_ITEM:
		if (KindCurrency <= 0) {
			cMessageBox.ShowMessageEvent("Selecione um tipo de moeda!");
			break;
		}

		if (KindCurrency == 1) {
			if (sMessageBox2[Kind].Count > 1000000000) {
				cMessageBox.ShowMessageEvent("Limite de venda de itens por creditos ultrapassado!");
				break;
			}

			MyShopMouseItem.sItemInfo.Coin = sMessageBox2[Kind].Count;
			cMyShop.LastSetMyShopItem(&MyShopMouseItem);
		}
		else if (cInvenTory.CheckMoneyLimit(sMessageBox2[Kind].Count)) {
	...
	case MESSAGE_MYSHOP_POTION:
		if (cCharShop.OpenFlag) {
			...
			sMyShop_Server.KindCurrency = KindCurrency;
			Send_PersonalShopItem(sRecvMyShop.CHAR_CODE, &sMyShop_Server);
		}
		break;
